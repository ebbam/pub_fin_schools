  ...
  df %>% 
    ...
    mutate(across(!c(unit_id, time_id),
                  list(t0 = ~(ifelse(!!sym(time_id) == 2001, 1, NA))*.,
                       t1 = ~(ifelse(!!sym(time_id) == 2002, 1, NA))*.,
                       t2 = ~(ifelse(!!sym(time_id) == 2003, 1, NA))*.,
                       t4 = ~(ifelse(!!sym(time_id) == 2004, 1, NA))*.),
                  .names = "{.fn}_{.col}")) %>% 
    group_by(!!sym(unit_id)) %>% 
    fill(matches("^t[[:digit:]]"),
      .direction = "down") %>% 
    mutate(across(!c(!!sym(time_id), matches("^t[[:digit:]]")), list(ma5 = ~rollmean(., 5, align = 'right', fill = NA),
    ...
    mutate(across(vars,
                  list(delta = ~ . - get(paste0("t0_", cur_column())),
                       delta_m2 = ~ . - get(paste0("t1_", cur_column())),
                       delta_m3 = ~. - get(paste0("t2_", cur_column())),
                       delta_m5 = ~. - get(paste0("t4_", cur_column()))),
                       .names = "{.fn}_{.col}")) %>% return(.)
}

rel_vars <- c("real_Elem_Educ_Total_Exp", "real_Elem_Educ_Total_Exp_pp", "real_Property_Tax","real_Property_Tax_pp", "log_real_Elem_Educ_Total_Exp", "log_real_Elem_Educ_Total_Exp_pp", "log_real_Property_Tax","log_real_Property_Tax_pp",
              "real_gdp_total", "real_gdp_total_pc", "log_real_gdp_total", "log_real_gdp_total_pc")
...
ggplot(filter(mines_state, year > 2001),
  ...
  geom_line(aes(y = diff_real_Elem_Educ_Total_Exp, colour = 'red'))+
  geom_line(aes(y= diff_real_gdp_priv_ind/20, colour = 'blue')) +
  facet_wrap(~state_name, scales = "free") +
  ...
  labs(title = "Plot of Elementary Education Expenditure and Private Industry GDP",
       subtitle = "in FD real USD by state",
       y = NULL)
...
ggplot(filter(mines_state, year > 2001),
  ...
  geom_line(aes(y = diff_real_Property_Tax, colour = 'red'))+
  geom_line(aes(y = diff_real_Elem_Educ_Total_Exp/5, colour = 'blue')) +
  facet_wrap(~state_name, scales = "free") +
...
ggplot(filter(mines_state, year > 2001),
  ...
  geom_line(aes(y = diff_real_Property_Tax, colour = 'red'))+
  geom_line(aes(y= diff_real_gdp_priv_ind/100, colour = 'blue')) +
  facet_wrap(~state_name, scales = "free") +
...
for(k in names(select(cz_trends_dat, contains('delta') & contains('log_real_Elem_Educ_Total_Exp_pp')))){
  cat('##### ', k, ' \n')
    p <- cz_trends_dat %>% 
      filter(state_name != "ME") %>% 
      ...
      geom_line() +
      geom_line(data = natl_trends_dat, aes(x = year, y = get(k)), colour = "black", linetype = "dashed") +
      geom_line(data = state_trends_dat, aes(x = year, y = get(k)), colour = "red", linetype = "dashed") +
      theme(legend.position = 'none') +
      scale_colour_viridis(discrete = TRUE) + 
      facet_wrap(~state_name, ncol = 4, scales = "free") +
      theme(legend.position = 'none') + 
...
natl_trend_pc <- mines_natl %>% 
  select(year, log_real_Elem_Educ_Total_Exp_pp) %>% 
  distinct %>% 
  lm(log_real_Elem_Educ_Total_Exp_pp ~ year, data = .) %>% 
  .$coefficients %>% pluck(2)
  
county_ed_trends = mines_restr %>% 
  group_by(fips, state_name) %>% 
  do(model = lm(log_real_Elem_Educ_Total_Exp_pp ~ year, data = .)) %>% 
  ungroup 

county_ed_trends_controlled_natl = mines_restr %>% 
  left_join(., mines_natl, by = "year") %>% 
  group_by(fips, state_name) %>% 
  do(model = lm(log_real_Elem_Educ_Total_Exp_pp.x ~ year +  diff_log_real_Elem_Educ_Total_Exp_pp.y, data = .)) %>% 
  ungroup 
...
grid.arrange(trend_ed_plot_pc, trend_ed_state_plot_pc, trend_ed_natl_plot_pc, ncol = 1) %>% suppressWarnings()
...
county_trends = mines_restr %>% group_by(fips, state_name) %>% do(model = lm(log_real_gdp_total ~ year, data = .)) %>% ungroup 

county_trends_controlled_natl = mines_restr %>% 
  left_join(., gdp_annual, by = "year") %>% 
  group_by(fips, state_name) %>% 
  do(model = lm(log_real_gdp_total ~ year + diff_log_gdp_natl, data = .)) %>% 
  ungroup 
...
county_trends_controlled_state = mines_restr %>% 
  left_join(., mines_state, by = c("year", "state_name")) %>% 
  group_by(fips, state_name) %>% 
  do(model = lm(log_real_gdp_total.x ~ year + diff_log_real_gdp_total.y, data = .)) %>% 
  ungroup 

state_trends = mines_state %>% group_by(state_name) %>% do(model = lm(log_real_gdp_total ~ year, data = .)) %>% ungroup

state_trends_controlled_natl = mines_state %>% 
  left_join(., gdp_annual, by = "year") %>% 
  group_by(state_name) %>% 
  do(model = lm(log_real_gdp_total ~ year + log_gdp_natl, data = .)) %>% 
  ungroup
...
county_trends_pc = mines_restr %>% 
  group_by(fips, state_name) %>% 
  do(model = lm(log_real_gdp_total_pc ~ year, data = .)) %>% ungroup

county_trends_controlled_natl_pc = mines_restr %>% 
  left_join(., gdp_annual, by = "year") %>% 
  group_by(fips, state_name) %>% 
  do(model = lm(log_real_gdp_total_pc ~ year + diff_log_gdp_natl_pc, data = .)) %>% 
  ungroup 

county_trends_controlled_state_pc = mines_restr %>% 
  left_join(., mines_state, by = c("year", "state_name")) %>% 
  group_by(fips, state_name) %>% 
  do(model = lm(log_real_gdp_total_pc.x ~ year + diff_log_real_gdp_total_pc.y, data = .)) %>% 
  ungroup 

state_trends_pc = mines_state %>% 
  group_by(state_name) %>% 
  do(model = lm(log_real_gdp_total_pc ~ year, data = .)) %>% 
  ungroup
...
natl_trend_pc = gdp_annual %>% 
  select(year, log_gdp_natl_pc) %>% 
  distinct %>% do(model = lm(log_gdp_natl_pc ~ year, data = .)) %>% 
  pull(model) %>% first %>% .$coefficients %>% pluck(2)
...
grid.arrange(trend_plot, trend_plot_pc, trend_state_plot, trend_state_plot_pc, trend_natl_plot,  trend_natl_plot_pc, ncol = 2) %>% suppressWarnings()
