...
mines_restr %>% 
  ...
  summarise(mean_share_own = mean(share_own, na.rm = TRUE), 
            ...
            mean_share_fed = mean(share_fed, na.rm = TRUE))  %>% 
  ungroup %>% 
  left_join(czs,., by = "cz_id") %>% 
  plot_usmap(data = ., values = "mean_share_own", regions = "counties", col = "gray90", linewidth = 0.01, exclude = c("AK", "HI")) +
  borders("states", color = "black", size = 0.3) +
  scale_fill_gradient2(midpoint = 0, na.value = "palegreen", low = "mediumvioletred", high = "midnightblue") +
  #scale_fill_distiller(colours=c(bl,"white", re), name = "", direction = 1, na.value = "gray90", limits = c(-1, 1)) +
  theme(panel.background = element_rect(color = "white", fill = "white"),
        plot.title = element_text(face = "bold")) +
  labs(title = "Share of Exp from Own Sources",
       fill = "Share of Exp from Own Sources (Mean 2001-2021)") 
