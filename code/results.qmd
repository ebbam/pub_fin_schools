...
df <- df_all %>% 
  ...
  ungroup %>% 
  # Correcting incorrect log values! need to do this in the underlying dataset
  mutate(test = log_Property_Tax_pp,
              across(c(log_Total_Revenue,
log_Total_Taxes,
log_Property_Tax,
log_Total_IG_Revenue,
log_Total_Fed_IG_Revenue,
log_Total_State_IG_Revenue,
log_Tot_Local_IG_Rev,
log_Total_Expenditure,
log_Total_Educ_Total_Exp,
log_Total_Rev_Own_Sources,
log_Total_Revenue_pp,
log_Total_Taxes_pp,
log_Property_Tax_pp,
log_Total_IG_Revenue_pp,
log_Total_Fed_IG_Revenue_pp,
log_Total_State_IG_Revenue_pp,
log_Tot_Local_IG_Rev_pp,
log_Total_Expenditure_pp,
log_Total_Educ_Total_Exp_pp,
log_Total_Rev_Own_Sources_pp), ~log((exp(.)*1000) + 1)))
...
df <- df %>% 
  ...
  group_by(fips) %>% 
  mutate(across(matches(paste0(names(select(gdp, -c(fips, year))), collapse = "|")),list(diff = ~. - dplyr::lag(., 1), log = ~log((.*1000) + 1)), .names = "{.fn}_{.col}"),
...
<!-- ```{r} -->
<!-- list(feols(log_Property_Tax ~ log_gdp_total + sw0(l(log_Property_Tax, 1)) | fips + year, data = mines_df, se = "cluster", cluster = "fips", panel.id = ~fips+year), -->
<!--      feols(log_Property_Tax_pp ~ log_gdp_total_pc + sw0(l(log_Property_Tax_pp, 1))| fips + year, data = mines_df, se = "cluster", cluster = "fips", panel.id = ~fips+year)) %>% etable(se.below = TRUE) %>% kable -->
...
list(feols(log_Total_Educ_Total_Exp_pp ~ l(log_Total_Educ_Total_Exp_pp, 1) + sw0(log_gdp_total_pc,
                                         log_gdp_priv_ind_pc,
                                         log_gdp_o_g_mining_quarr_21_pc) + time:as.factor(state) | fips + year, data = mines_df, se = "cluster", cluster = "fips", panel.id = ~fips+year),
