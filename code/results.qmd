...
# # From defining_decline.qmd
# test <- readRDS(here("data/out/regression_data_complete_fips.RDS")) %>%
#   mutate(across(all_of(c(vars_grab_restr, paste0("log_", vars_grab_restr))), ~. - dplyr::lag(., 1), .names = "diff_{.col}")) %>%
#   group_by(year) %>%
