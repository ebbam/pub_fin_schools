...
We measure the "residual" growth trends in the following way. 1. Without controls: model the univariate case $\Delta log(E_{it}) = 1 + \epsilon_{it}$

2.  Controlling for state trend: $\Delta log(E_{it}) = \Delta log(E_{st}) + \epsilon_{it}$

3.  Controlling for national trend: $\Delta log(E_{it}) = \Delta log(E_{t}) + \epsilon_{it}$
...
cz_map_data <- cz_to_map(mutate(rename(county_ed_coefs, cz_id = unit), year = 2001), gen_czs_years(rename(mutate(county_ed_coefs, year = 2001), cz_id = unit), czs)) 

plot_usmap(data = cz_map_data, values = "trend_cont_natl", regions = "counties", col = "gray90", linewidth = 0.01, exclude = c("AK", "HI")) +
  scale_fill_gradient2(midpoint = 0, na.value = "grey98") +
  #scale_fill_distiller(colours=c(bl,"white", re), name = "", direction = 1, na.value = "gray90", limits = c(-1, 1)) +
  theme(panel.background = element_rect(color = "white", fill = "white"),
        plot.title = element_text(face = "bold"), legend.background=element_blank(), legend.title = element_blank()) +
  labs(title = "Trend (Overall)") 

plot_usmap(data = cz_map_data, values = "trend_cont_state", regions = "counties", col = "gray90", linewidth = 0.01, exclude = c("AK", "HI")) +
  scale_fill_gradient2(midpoint = 0, na.value = "grey98") +
  #scale_fill_distiller(colours=c(bl,"white", re), name = "", direction = 1, na.value = "gray90", limits = c(-1, 1)) +
  theme(panel.background = element_rect(color = "white", fill = "white"),
        plot.title = element_text(face = "bold"), legend.background=element_blank(), legend.title = element_blank()) +
  labs(title = "Trend Controlling for State") 
...
plot_usmap(data = cz_map_data, values = "trend_cont_natl", regions = "counties", col = "gray90", linewidth = 0.01, exclude = c("AK", "HI")) +
  ...
  theme(panel.background = element_rect(color = "white", fill = "white"),
        plot.title = element_text(face = "bold"), legend.background=element_blank(), legend.title = element_blank()) +
  labs(title = "Trend Controlling for National") 
...
\Delta log(E)_{it}
= \beta_{0}
+ \beta_{N}\,\mathrm{\Delta log(E)}_{t}^{national}
+ \beta_{S}\,\mathrm{\Delta log(E)}_{it}^{state}
\;+\;
b_{0i}
+ b_{Ni}\,\mathrm{\Delta log(E)}_{t}^{national}
+ b_{Si}\,\mathrm{\Delta log(E)}_{it}^{state}
\;+\;
\varepsilon_{it},
$$
...
# 1. Stack national, state, and county data into one panel
panel <- mines_restr %>%
  left_join(mines_natl   %>% select(year, nat_trend   = diff_log_real_Elem_Educ_Total_Exp_pp),
            by = "year") %>%
  left_join(mines_state %>% select(year, state, state_trend = diff_log_real_Elem_Educ_Total_Exp_pp),
            by = c("year", "state"))

# 2. Fit the mixed‐effects (“factor”) model
mod <- lmer(
  diff_log_real_Elem_Educ_Total_Exp_pp ~ 
    nat_trend + 
    state_trend + 
    (1 + nat_trend + state_trend | unit),
  data = panel, 
  REML = FALSE
)
...
beta0   <- fe["(Intercept)"]
betaN   <- fe["nat_trend"]
betaS   <- fe["state_trend"]

# 4. Extract county‐level random deviations
re_unit <- ranef(mod)$unit %>%
  tibble::rownames_to_column("unit") %>%
  rename(
    local_int_dev   = `(Intercept)`,
    nat_slope_dev   = nat_trend,
    state_slope_dev = state_trend
  )

# 5. Compute each county’s factor loadings
county_factors <- re_unit %>%
  mutate(
    local_factor     = beta0 + local_int_dev,
    nat_sensitivity  = betaN + nat_slope_dev,
    state_sensitivity = betaS + state_slope_dev
  )

# 6a. Plot sensitivities: national vs. state
ggplot(county_factors, aes(x = nat_sensitivity, y = state_sensitivity)) +
  geom_point(alpha = 0.6) +
  geom_vline(xintercept = betaN, linetype = "dashed", colour = "grey50") +
  geom_hline(yintercept = betaS, linetype = "dashed", colour = "grey50") +
  labs(
    x = "County Sensitivity to National Trend",
    y = "County Sensitivity to State Trend",
    title = "County Co‐movement with National & State Education Spending Trends"
  ) +
  theme_minimal()
# 

panel_contribs <- panel %>%
  left_join(county_factors, by = "unit") %>%
  mutate(
    contr_local     = local_factor,
    contr_national  = nat_sensitivity  * nat_trend,
    contr_state     = state_sensitivity * state_trend
  )
county_contribs_time <- panel_contribs %>%
  group_by(unit) %>%
  summarize(
    contrib_local    = mean(contr_local,    na.rm = TRUE),
    contrib_national = mean(contr_national, na.rm = TRUE),
    contrib_state    = mean(contr_state,    na.rm = TRUE)
  ) %>%
  pivot_longer(
    cols      = -unit,
    names_to  = "source",
...
# # 6b. Compute average contributions at mean trends
# mean_nat <- mean(panel$nat_trend, na.rm = TRUE)
# mean_st  <- mean(panel$state_trend, na.rm = TRUE)
# 
# county_contribs <- county_factors %>%
#   transmute(
#     unit,
#     contrib_local    = local_factor,
#     contrib_national = nat_sensitivity  * mean_nat,
#     contrib_state    = state_sensitivity * mean_st
#   ) %>%
#   pivot_longer(-unit, names_to = "source", values_to = "contribution")

# 6c. Plot stacked contributions (one bar per county, ordered by total)
ggplot(county_contribs_time, aes(x = reorder(unit, -contribution), y = contribution, fill = source)) +
  geom_col() +
  labs(
    x = NULL,
    y = "Avg per‐year Δ log per‐pupil spending",
    fill = "Driver",
    title = "Decomposed County‐Level Growth Drivers"
  ) +
  theme_minimal() +
  ...
  scale_fill_brewer(palette = "Set2") +
    ...
    name = "Driver",
    values = c(
      "contrib_state" = "#66c2a5",
      "contrib_national" = "#fc8d62",
      "contrib_local" = "#8da0cb"
    ),
    labels = c(
      "contrib_state" = "State Factors",
      "contrib_national" = "National Factor",
      "contrib_local" = "Local Factors"
    )
  )


model <- feols(
  diff_log_real_Elem_Educ_Total_Exp_pp ~ 1 | unit + year + state^year,
  data = mines_cz,
  cluster = "unit"  # clustered standard errors at unit level
)
