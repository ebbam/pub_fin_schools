---
title: "Appendix: Property Prices"
format:
  pdf:
...
```{r, echo = FALSE, include = FALSE, message = FALSE, cache = FALSE}
...
iv_fitstats = ~ n + ivfall + ivfall.p + wh + wh.p + ivwaldall + ivwaldall.p
...
The structure of public financing (described in
further detail in Section \ref{si_section:Challenges} of the
Supplementary Materials) provides an avenue for
a causal identification strategy. In brief, although revenue for public education comes from a combination of intergovernmental and local sources; revenue
...
t1 <- feols(log_hpi ~ l(log_weighted_annual_avg_wkly_wage, 0:7) + log_real_gdp_priv_ind_pc + log_pop_total | unit + year, panel.id = c('year', 'unit'), mines_cz, cluster = "unit")
t1_state_fe <- feols(log_hpi ~ l(log_weighted_annual_avg_wkly_wage, 0:7) + log_real_gdp_priv_ind_pc + log_pop_total | state + year, panel.id = c('year', 'unit'), mines_cz, cluster = "unit")

t2 <- feols(gr_hpi ~ l(gr_weighted_annual_avg_wkly_wage, 0:7) + diff_log_real_gdp_priv_ind_pc + diff_log_pop_total| unit + year, panel.id = c('year', 'unit'), mines_cz, cluster = "unit")
t2_state_fe <- feols(gr_hpi ~ l(gr_weighted_annual_avg_wkly_wage, 0:7) + diff_log_real_gdp_priv_ind_pc + diff_log_pop_total| state + year, panel.id = c('year', 'unit'), mines_cz, cluster = "unit")
...
iv_lev_form <- "log_real_Elem_Educ_Total_Exp_pp ~  l1_log_real_Elem_Educ_Total_Exp_pp + log_real_Total_IG_Revenue_pp + log_real_gdp_priv_ind_pc + log_Enrollment + pct_black + pct_hispanic"
...
iv_model_2d_lev_gr <- feols(as.formula(paste0(iv_lev_form, " | unit + year | log_hpi ~ ss_2d")),
    data = left_join(mines_cz, rename(ss_temp, unit = cz_id), by = c("year","unit")), panel.id = c("unit", "year"), cluster = "unit")

iv_model_2d_lev_lev <- feols(
    as.formula(paste0(iv_lev_form, " | unit + year | log_hpi ~ lev_ss_2d")),
    data = left_join(mines_cz, rename(ss_temp, unit = cz_id), by = c("year","unit")), panel.id = c("unit", "year"), cluster = "unit")

iv_model_2d_lev_gr_l1 <- feols(as.formula(paste0(iv_lev_form, " | unit + year | log_hpi ~ l(ss_2d,1)")),
    data = left_join(mines_cz, rename(ss_temp, unit = cz_id), by = c("year","unit")), panel.id = c("unit", "year"), cluster = "unit")

iv_model_2d_lev_lev_l1 <- feols(
    as.formula(paste0(iv_lev_form, " | unit + year | log_hpi ~ l(lev_ss_2d,1)")),
    data = left_join(mines_cz, rename(ss_temp, unit = cz_id), by = c("year","unit")), panel.id = c("unit", "year"), cluster = "unit")

iv_model_2d_lev_gr_state_fe <- feols(as.formula(paste0(iv_lev_form, " | state + year | log_hpi ~ ss_2d")),
    data = left_join(mines_cz, rename(ss_temp, unit = cz_id), by = c("year","unit")), panel.id = c("unit", "year"), cluster = "unit")

iv_model_2d_lev_lev_state_fe <- feols(
    as.formula(paste0(iv_lev_form, " | state + year | log_hpi ~ lev_ss_2d")),
    data = left_join(mines_cz, rename(ss_temp, unit = cz_id), by = c("year","unit")), panel.id = c("unit", "year"), cluster = "unit")

iv_model_2d_lev_gr_l1_state_fe <- feols(as.formula(paste0(iv_lev_form, " | state + year | log_hpi ~ l(ss_2d,1)")),
    data = left_join(mines_cz, rename(ss_temp, unit = cz_id), by = c("year","unit")), panel.id = c("unit", "year"), cluster = "unit")

iv_model_2d_lev_lev_l1_state_fe <- feols(
    as.formula(paste0(iv_lev_form, " | state + year | log_hpi ~ l(lev_ss_2d,1)")),
    data = left_join(mines_cz, rename(ss_temp, unit = cz_id), by = c("year","unit")), panel.id = c("unit", "year"), cluster = "unit")

# iv_model_2d_lev_gr_tfe <- feols(as.formula(paste0(iv_lev_form, " | year + state | log_hpi ~ l(ss_2d,1)"),
#     data = left_join(mines_cz, rename(ss_temp, unit = cz_id), by = c("year","unit")), panel.id = c("unit", "year"))
# 
# iv_model_2d_lev_lev_tfe <- feols(
#     as.formula(paste0(iv_lev_form, " | year + state | log_hpi ~ l(lev_ss_2d,1)"),
#     data = left_join(mines_cz, rename(ss_temp, unit = cz_id), by = c("year","unit")), panel.id = c("unit", "year"))
...
t1 <- feols(log_hpi ~ l(log_real_gdp_priv_ind_pc, 0:7) + log_pop_total | unit + year, panel.id = c('year', 'unit'), mines_cz, cluster = "unit")
t1_state_fe <- feols(log_hpi ~ l(log_real_gdp_priv_ind_pc, 0:7) + log_pop_total | state + year, panel.id = c('year', 'unit'), mines_cz, cluster = "unit")

t2 <- feols(gr_hpi ~ l(diff_log_real_gdp_priv_ind_pc, 0:7) +  diff_log_pop_total| unit + year, panel.id = c('year', 'unit'), mines_cz, cluster = "unit")
t2_state_fe <- feols(gr_hpi ~ l(diff_log_real_gdp_priv_ind_pc, 0:7) +  diff_log_pop_total| state + year, panel.id = c('year', 'unit'), mines_cz, cluster = "unit")
...
iv_model_2d_lev_gr <- feols(as.formula(paste0(iv_lev_form, " | unit + year | log_hpi ~ gdp_ss_2d")),
    data = left_join(mines_cz, rename(ss_temp, unit = cz_id), by = c("year","unit")), panel.id = c("unit", "year"), cluster = "unit")

iv_model_2d_lev_lev <- feols(
    as.formula(paste0(iv_lev_form, " | unit + year | log_hpi ~ l1_log_hpi + lev_gdp_ss_2d + l1_lev_gdp_ss_2d + l2_lev_gdp_ss_2d")),
    data = df_ivs, panel.id = c("unit", "year"), cluster = "unit")

iv_model_2d_lev_lev_no_ar <- feols(
    as.formula(paste0(iv_lev_form, " | unit + year | log_hpi ~ lev_gdp_ss_2d + l1_lev_gdp_ss_2d + l2_lev_gdp_ss_2d")),
    data = df_ivs, panel.id = c("unit", "year"), cluster = "unit")

etable(iv_model_2d_lev_lev, iv_model_2d_lev_lev_no_ar, adjustbox = TRUE, stage = 1:2, fitstat = iv_fitstats, tex = latex_tables, title ="GDP-based Shift-share instrument (Level-Level, varying state and CZ fixed effects and lags.")

iv_model_2d_lev_gr_l1 <- feols(as.formula(paste0(iv_lev_form, " | unit + year | log_hpi ~ l(gdp_ss_2d,1)")),
    data = left_join(mines_cz, rename(ss_temp, unit = cz_id), by = c("year","unit")), panel.id = c("unit", "year"), cluster = "unit")

iv_model_2d_lev_lev_l1 <- feols(
    as.formula(paste0(iv_lev_form, " | unit + year | log_hpi ~ l(lev_gdp_ss_2d,1)")),
    data = left_join(mines_cz, rename(ss_temp, unit = cz_id), by = c("year","unit")), panel.id = c("unit", "year"), cluster = "unit")

iv_model_2d_lev_gr_state_fe <- feols(as.formula(paste0(iv_lev_form, " | state + year | log_hpi ~ gdp_ss_2d")),
    data = left_join(mines_cz, rename(ss_temp, unit = cz_id), by = c("year","unit")), panel.id = c("unit", "year"), cluster = "unit")

iv_model_2d_lev_lev_state_fe <- feols(
    as.formula(paste0(iv_lev_form, " | state + year | log_hpi ~ lev_gdp_ss_2d")),
    data = left_join(mines_cz, rename(ss_temp, unit = cz_id), by = c("year","unit")), panel.id = c("unit", "year"), cluster = "unit")

iv_model_2d_lev_gr_l1_state_fe <- feols(as.formula(paste0(iv_lev_form, " | state + year | log_hpi ~ l(gdp_ss_2d,1)")),
    data = left_join(mines_cz, rename(ss_temp, unit = cz_id), by = c("year","unit")), panel.id = c("unit", "year"), cluster = "unit")

iv_model_2d_lev_lev_l1_state_fe <- feols(
    as.formula(paste0(iv_lev_form, " | state + year | log_hpi ~ l(lev_gdp_ss_2d,1)")),
    data = left_join(mines_cz, rename(ss_temp, unit = cz_id), by = c("year","unit")), panel.id = c("unit", "year"), cluster = "unit")
...
```{r, echo = FALSE, fig.height = 10, include = FALSE, fig.width = 10, warning = FALSE, cache = FALSE}
...
```{r, echo = FALSE, include = FALSE, fig.height = 10, fig.width = 10, warning = FALSE, cache = FALSE}
...
```{r, echo = FALSE, fig.height = 10, fig.width = 10,  include = FALSE, warning = FALSE, cache = FALSE}
...
##### State-by-state and industry-by-industry estimation
...
for (k in temp_vars) {
  ...
  iv_model <- feols(
    ...
    data = dat_temp, panel.id = c("unit", "year"), cluster = "unit"
...
for(k in reg_states){
  mod <- dat_temp %>%
    filter(state == k) %>%
    feols(as.formula(paste0(iv_lev_form, " | unit + year | log_hpi ~ l(gdp_ss_2d,1)")),
    data = ., panel.id = c("unit", "year"), cluster = "unit")
...
for(k in reg_states){
  mod <- dat_temp %>%
    filter(state == k) %>%
    feols(as.formula(paste0(iv_lev_form, " | unit + year | log_hpi ~ l(ss_2d,1)")),
    data = ., panel.id = c("unit", "year"), cluster = "unit")
  state_mods_ss[[as.character(k)]] <- mod
}
