---
title: "Effect of Industrial Shocks on Public Ed."
format: 
  html:
...
mean_df <- mines_cz %>% 
  group_by(year) %>%
             summarise(Total_Educ_Total_Exp = mean(Total_Educ_Total_Exp, na.rm = TRUE),
                       ...
                       log_Total_Educ_Total_Exp = mean(log_Total_Educ_Total_Exp, na.rm = TRUE),
                       log_Total_Educ_Total_Exp_pp = mean(log_Total_Educ_Total_Exp_pp, na.rm = TRUE))
...
mines_cz %>%
  select(Enrollment, pop_total, Elem_Educ_Total_Exp_pp, Property_Tax_pp, Total_IG_Revenue_pp, Total_Fed_IG_Revenue_pp, 
         Total_State_IG_Revenue_pp, gdp_total_pc, gdp_priv_ind_pc, gdp_o_g_mining_quarr_21_pc,
         Elem_Educ_Total_Exp, Property_Tax, Total_IG_Revenue, Total_Fed_IG_Revenue, 
         Total_State_IG_Revenue, gdp_total, gdp_priv_ind, gdp_o_g_mining_quarr_21, total_active_n, total_active_prod) %>% 
  mutate(across(c(gdp_total, gdp_priv_ind, gdp_o_g_mining_quarr_21, gdp_total_pc, gdp_priv_ind_pc, gdp_o_g_mining_quarr_21_pc))) %>%  # gdp_govt, 
          data.frame() %>%
...
state_lev <- mines_cz %>% 
  group_by(main_state, year) %>% 
  summarise(across(c(gdp_priv_ind, gdp_total, pop_total), ~sum(., na.rm = TRUE))) %>% 
  ungroup %>% 
  mutate(gdp_priv_ind_pc = gdp_priv_ind/pop_total,
         gdp_total_pc = gdp_total/pop_total,
         across(!c(main_state, year), ~log(. + 1), .names = "log_{.col}")) %>% 
  group_by(main_state) %>% 
...
growth_rates <- mines_cz %>% 
  select(cz_id, names(state_lev)) %>%
  left_join(., state_lev, by = c("main_state", "year"), suffix = c("", "_state")) %>% 
...
trend_forms <- c("trend" = "diff_log_gdp_total ~ diff_log_gdp_total_state + diff_log_gdp_total_natl", 
                "trend_pc" = "diff_log_gdp_total_pc ~ diff_log_gdp_total_pc_state + diff_log_gdp_total_pc_natl")
                # "trend_pc" = "diff_log_gdp_total_pc ~ diff_log_gdp_total_pc_state")
                 #"trend_pc" = "diff_log_gdp_total_pc ~ diff_log_gdp_total_pc_natl")
...
cz_trends_plot <- cz_trends %>% 
  left_join(., distinct(select(mines_cz, cz_id, main_state)), by = "cz_id")

cz_trends_plot$state = sapply(cz_trends_plot$main_state, get_state) 
...
cz_states <- cz_trends_plot %>% 
  ...
  mutate(cz_id = factor(cz_id, levels = cz_id)) %>%
  ggplot(aes(cz_id, trend_pc)) +
  geom_segment(aes(x=cz_id ,xend=cz_id, y=0, yend=trend_pc, color = state)) +
...
cz_trend <- cz_trends_plot %>% 
  ...
  mutate(cz_id = factor(cz_id, levels = cz_id)) %>% 
  ggplot(aes(cz_id, trend_pc)) +
  geom_segment(aes(x=cz_id ,xend=cz_id, y=0, yend=trend_pc, color = cz_id)) +
  geom_point( color="darkblue", size=0.5) +
  coord_flip() +
...
cz_regions <- cz_trends_plot %>% 
  ...
  mutate(cz_id = factor(cz_id, levels = cz_id)) %>%
  ggplot(aes(cz_id, trend_pc)) +
  geom_segment(aes(x=cz_id ,xend=cz_id, y=0, yend=trend_pc, color = region)) +
  geom_point( color="darkblue", size=0.5) +
...
mines_cz_decl <- cz_trends %>% 
  select(cz_id, contains("trend"), contains("growing"), contains("declining")) %>% 
  left_join(mines_cz, ., by = "cz_id")
...
mines_no_outliers <- mines_cz %>% 
  group_by(cz_id) %>% 
  filter(!any(log_Elem_Educ_Total_Exp_pp > 10.25)) %>% 
  ungroup

mines_cz%>% 
  ggplot(aes(x = log_Elem_Educ_Total_Exp_pp, y = log_Property_Tax_pp, colour = share_own_discrete)) +
  geom_jitter() +
...
#   pull(cz_id) %>% 
#   unique %>% 
#   setdiff(unique(mines_cz$cz_id), .)

ss_df <- mines_cz %>% 
  left_join(., ss, by = c("year", "cz_id"))
...
sswage <- #ss_df %>% 
  ss %>% 
  dplyr::select(cz_id, year, ss_emp_coal_2001, ss_emp_oil_gas_2001, ss_emp_extraction_2001) %>% 
  pivot_longer(contains("ss")) %>% 
  group_by(cz_id) %>% 
  filter(any(!is.na(value))) %>% 
  ungroup %>% 
  ggplot(aes(x = year, y = value, group = cz_id)) +
...
ss_coal <- #ss_df %>%
  ss %>% 
  dplyr::select(cz_id, year,ss_wage_coal_2001, ss_wage_oil_gas_2001, ss_wage_extraction_2001) %>% 
  pivot_longer(contains("ss")) %>% 
  group_by(cz_id) %>% 
  filter(any(!is.na(value))) %>% 
  ungroup %>% 
...
<!--   ss_coal_instruments_res  <- append(paste0("log_Total_Educ_Total_Exp_pp ~ log_Total_State_IG_Revenue_pp + log_Total_Fed_IG_Revenue_pp | cz_id + year | log_Property_Tax_pp ~ ", ss), ss_coal_instruments_res) -->
<!-- } -->
...
for(ss in ss_oil_gas_instruments){
  ss_oil_gas_instruments_res  <- append(paste0("log_Total_Educ_Total_Exp_pp ~ log_Total_State_IG_Revenue_pp + log_Total_Fed_IG_Revenue_pp | cz_id + year | log_Property_Tax_pp ~ ", ss), ss_oil_gas_instruments_res)
} 
...
for(ss in ss_ff_instruments){
  ss_ff_instruments_res  <- append(paste0("log_Total_Educ_Total_Exp_pp ~ log_Total_State_IG_Revenue_pp + log_Total_Fed_IG_Revenue_pp | cz_id + year | log_Property_Tax_pp ~ ", ss), ss_ff_instruments_res)
} 
...
for(ss in ss_extraction_instruments){
  ss_extraction_instruments_res  <- append(paste0("log_Total_Educ_Total_Exp_pp ~ log_Total_State_IG_Revenue_pp + log_Total_Fed_IG_Revenue_pp | cz_id + year | log_Property_Tax_pp ~ ", ss), ss_extraction_instruments_res)
} 
...
spv_cz <- spv_fips %>% 
  ...
  group_by(year, cz_id) %>% 
         summarise(across(c(p_area, p_cap_ac, p_cap_dc), ~sum(., na.rm = TRUE), .names = "pv_{.col}"))

mines_cz_solar <- left_join(mines_cz, spv_cz, by = c("cz_id", "year")) %>% 
  group_by(cz_id) %>% 
...
# run_model(solar_iv_models, mines_cz_solar) %>% 
#   etable()
source(here('code/reg_forms.R'))

run_model(solar_iv_models_pc, mines_cz_solar) %>% 
  etable()
...
<!-- mines_fips_solar <- left_join(mines_restr, spv_fips, by = c("fips", "year")) %>%  -->
<!--   group_by(fips) %>%  -->
<!--   fill(pv_p_area, .direction = "down") %>%  -->
<!--   fill(pv_p_cap_ac, .direction = "down") %>%  -->
<!--   fill(pv_p_cap_dc, .direction = "down") %>%  -->
...
<!-- solar_iv_models = c(log_Elem_Educ_Total_Exp_pp ~ log_Total_State_IG_Revenue_pp + log_Total_Fed_IG_Revenue_pp | fips + year | log_Property_Tax_pp ~ pv_p_area, -->
<!--                        log_Elem_Educ_Total_Exp_pp ~ log_Total_State_IG_Revenue_pp + log_Total_Fed_IG_Revenue_pp | fips + year | log_Property_Tax_pp ~ pv_p_cap_ac, -->
<!--                     log_Elem_Educ_Total_Exp_pp ~ log_Total_State_IG_Revenue_pp + log_Total_Fed_IG_Revenue_pp | fips + year | log_Property_Tax_pp ~ pv_p_cap_dc, -->
<!--                        log_Elem_Educ_Total_Exp_pp ~ log_Total_State_IG_Revenue_pp + log_Total_Fed_IG_Revenue_pp | fips + year | log_gdp_total_pc ~ pv_p_area, -->
<!--                        log_Elem_Educ_Total_Exp_pp ~ log_Total_State_IG_Revenue_pp + log_Total_Fed_IG_Revenue_pp | fips + year | log_gdp_total_pc ~ pv_p_cap_ac, -->
<!--                     log_Elem_Educ_Total_Exp_pp ~ log_Total_State_IG_Revenue_pp + log_Total_Fed_IG_Revenue_pp | fips + year | log_gdp_total_pc ~ pv_p_cap_dc) -->

<!-- solar_iv_models_pc = c(log_Elem_Educ_Total_Exp_pp ~ log_Total_State_IG_Revenue_pp + log_Total_Fed_IG_Revenue_pp | fips + year | log_Property_Tax_pp ~ pv_p_area_pc, -->
<!--                     log_Elem_Educ_Total_Exp_pp ~ log_Total_State_IG_Revenue_pp + log_Total_Fed_IG_Revenue_pp | fips + year | log_Property_Tax_pp ~ pv_p_cap_ac_pc, -->
<!--                     log_Elem_Educ_Total_Exp_pp ~ log_Total_State_IG_Revenue_pp + log_Total_Fed_IG_Revenue_pp | fips + year | log_Property_Tax_pp ~ pv_p_cap_dc_pc, -->
<!--                     log_Elem_Educ_Total_Exp_pp ~ log_Total_State_IG_Revenue_pp + log_Total_Fed_IG_Revenue_pp | fips + year | log_gdp_total_pc ~ pv_p_area_pc, -->
<!--                     log_Elem_Educ_Total_Exp_pp ~ log_Total_State_IG_Revenue_pp + log_Total_Fed_IG_Revenue_pp | fips + year | log_gdp_total_pc ~ pv_p_cap_ac_pc, -->
<!--                     log_Elem_Educ_Total_Exp_pp ~ log_Total_State_IG_Revenue_pp + log_Total_Fed_IG_Revenue_pp | fips + year | log_gdp_total_pc ~ pv_p_cap_dc_pc) -->

<!-- run_model_fips(solar_iv_models, mines_fips_solar) %>%  -->
<!--   etable() -->

<!-- run_model_fips(solar_iv_models_pc, mines_fips_solar) %>%  -->
<!--   etable() -->
...
<!-- spv <- read.csv(here('data/raw/us_large_scale_solar_pv/uspvdbCSV/uspvdb_v2_0_20240801.csv')) %>%  -->
<!--   tibble %>%  -->
<!--   mutate(p_county = ifelse(p_state == "CA" & p_county == "San" & p_name == "Lockhart Solar PV II", "San Bernardino", p_county)) %>%  -->
<!--   mutate(fips = as_fips(p_state, p_county), -->
<!--          fips = case_when(fips == "12" & p_state == "FL" & p_county == "St Lucie" ~ "12111", -->
...
<!--     rename(year = p_year) %>%  -->
<!--   group_by(fips) %>%  -->
<!--   complete(case_id, year = 2000:2023, fill = list(NA)) %>%  -->
<!--   left_join(., czs, by = c("fips"), multiple = "first") %>%  -->
<!--   group_by(case_id) %>%  -->
<!--   fill(p_area, .direction = "down") %>%  -->
<!--   fill(p_cap_ac, .direction = "down") %>%  -->
<!--   fill(p_cap_dc, .direction = "down") %>%  -->
<!--   ungroup %>%  -->
<!--   group_by(year, cz_id) %>%  -->
<!--          summarise(across(c(p_area, p_cap_ac, p_cap_dc), ~sum(., na.rm = TRUE), .names = "pv_{.col}")) -->

<!-- mines_cz_solar <- left_join(mines_cz, spv, by = c("cz_id", "year")) %>%  -->
<!--   group_by(cz_id) %>%  -->
<!--   fill(pv_p_area, .direction = "down") %>%  -->
<!--   fill(pv_p_cap_ac, .direction = "down") %>%  -->
<!--   fill(pv_p_cap_dc, .direction = "down") %>%  -->
