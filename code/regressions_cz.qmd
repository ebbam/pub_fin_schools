...
Education expenditure has a highly relevant time dependence. The effect of increases in GDP two years prior has the greatest effect on current education expenditure, implying a delayed effect of commuting zone-level economic growth on public education expenditure.
...
ss_wage_instruments = c(#ss_wage_coal_2001, 
   #ss_wage_coal_2005, 
   #ss_wage_coal_2011,
...
ss_wage_forms <- list()
for(ss in ss_wage_instruments){
  ss_wage_forms <- append(paste0("log_Total_Educ_Total_Exp_pp ~ log_Total_State_IG_Revenue_pp + log_Total_Fed_IG_Revenue_pp | cz_id + year | log_Property_Tax_pp ~ ", ss), ss_wage_forms)
} 

run_model(ss_wage_forms, scaled_ss) %>% etable(headers = ss_wage_instruments)
...
Excluded as nothing is significant here!

```{r, include = FALSE}

ss_emp_instruments = c(
  "ss_emp_coal_2001",
  "ss_emp_coal_2005",
  "ss_emp_coal_2011",
  "ss_emp_oil_gas_2001",
...
ss_emp_forms <- list()
for(ss in ss_emp_instruments){
  ss_emp_forms <- append(paste0("log_Total_Educ_Total_Exp_pp ~ log_Total_State_IG_Revenue_pp + log_Total_Fed_IG_Revenue_pp | cz_id + year | log_Property_Tax_pp ~ ", ss), ss_emp_forms)
} 

run_model(ss_emp_forms, scaled_ss) %>% etable(headers = ss_emp_instruments)
