...
1.  Converted the Bartik instruments using wage and employment shares in various fossil fuel industries to commuting zone level to test pass-through via property taxes and local GDP.    
...
wages <- readRDS(here("data/raw/QCEW/QCEW_wage_stats_cz.RDS")) %>% 
  ...
  group_by(cz_id) %>% 
  arrange(year) %>% 
  mutate(across(contains("log"), ~. - lag(., 1), .names = "gr_{.col}")) %>% 
  rename(unit = cz_id)
...
# =======================
fe_model <- feols(
  log_real_Total_Educ_Total_Exp_pp ~ log_real_gdp_total_pc + log_weighted_annual_avg_wkly_wage | unit + year,
  data = mines_cz
)
...
# =======================
# 2. SHIFT-SHARE IV REGRESSION
# =======================
source(here("data/raw/QCEW/industry_shares_cleaning.R"))
ss_inst <- compute_ss(source = "QCEW", base_year = 2001, unit_id = "cz_id") %>% 
  plot_ss(., "52", "gr_natl_annual_avg_wkly_wage_", "cz_id", ind_codes)

# Assume you have:
# - industry_share_XX: share of local employment in industry XX
# - natl_growth_XX: national growth in industry XX
# Let's compute the shift-share instrument:

# IV regression using shift-share instrument
iv_model <- feols(
  log_real_Total_Educ_Total_Exp_pp ~ log_real_gdp_total_pc | unit + year | ss_52 ~ log_real_Property_Tax_pp,
  data = left_join(mines_cz, rename(ss_inst, unit = cz_id), by = c("year", "unit"))
)
...
# Assume you have already lagged variables created like:
# local_gdp_growth_lag1, local_gdp_growth_lag2, etc.

# Run distributed lag model
dl_model <- feols(
   log_real_Total_Educ_Total_Exp_pp ~ diff_log_real_gdp_total_pc + l(diff_log_real_gdp_total_pc) + l(diff_log_real_gdp_total_pc,2) +
    log_weighted_annual_avg_wkly_wage + l(log_weighted_annual_avg_wkly_wage) | unit + year, 
...
# remotes::install_github("joaopalmeiro/panelvar")

# Prepare your dataset
# Required: 'id' column (unit), 'time' column (year), and variables to include
pvar_data <- mines_cz %>%
...
pvar_model <- pvarfeols(
  dependent_vars = c("log_real_Total_Educ_Total_Exp", "log_real_gdp_total", "log_real_Property_Tax", "log_real_Total_State_IG_Revenue", "log_real_Total_Fed_IG_Revenue", "log_weighted_annual_avg_wkly_wage"),
  lags = 4,
...
irf_result <- pvar_model %>% girf(n.ahead = 7, ma_approx_steps= 7)

# Plot IRFs
plot(irf_result, scales = "free")
...
# ================================
# 1. HIERARCHICAL FIXED EFFECTS MODEL
# ================================

# This model decomposes variation in CZ-level GDP growth
# into national, state-level, and region-level time effects

# Estimate fixed effects model
fe_model <- feols(
  diff_log_real_gdp_total_pc ~ 1 |                      # No independent vars, just fixed effects
    year +                                     # National time trend
    interaction(state, year),                 # State-specific time trends
  data = mines_cz
)
...
# You can extract fixed effects for inspection
fixed_effects <- fixef(fe_model)

# Optionally: compare nested models to see variance explained
# Model 1: National only
fe_national <- feols(diff_log_real_gdp_total_pc ~ 1 | year, data = mines_cz)

# Model 2: National + State
fe_national_state <- feols(diff_log_real_gdp_total_pc ~ 1 | year + interaction(state, year), data = mines_cz)

# Model 3: National + State + Region
fe_full <- fe_model

# Compare R-squared values to see how much each level explains
etable(fe_national, fe_national_state, fe_full, se = "standard")

# ================================
# 2. FACTOR MODEL USING PCA
# ================================

# Goal: extract latent factors explaining common variation
# in local GDP growth across commuting zones

# # Convert mines_cz to wide format: one row per year, one column per CZ
# gdp_matrix <- mines_cz %>%
#   select(year, unit, diff_log_real_gdp_total_pc) %>%
#   pivot_wider(names_from = unit, values_from = diff_log_real_gdp_total_pc) %>%
#   arrange(year) %>%                         # ensure proper time order
#   select(-year) %>%                         # PCA doesn't need year column
#   scale(center = TRUE, scale = TRUE)        # standardize for PCA
# 
# # Perform principal component analysis
# pca_result <- prcomp(gdp_matrix, scale. = FALSE)  # already scaled above
# 
# # Plot scree plot to visualize variance explained
# plot(pca_result, type = "lines", main = "Scree Plot of Principal Components")
# 
# # Print proportion of variance explained by each PC
# summary(pca_result)
# 
# # Extract first few factors (e.g., first 2)
# factors <- as.data.frame(pca_result$x[, 1:2])
# colnames(factors) <- c("Factor1", "Factor2")
# 
# # Add year back in (optional)
# factors$year <- unique(mines_cz$year)

# You can now correlate these with national or state-level aggregates
# to see if Factor1 aligns with national GDP trends, etc.

# Example: correlate Factor1 with national GDP growth
# df_national has year and national_gdp_growth
# cor(factors$Factor1, df_national$national_gdp_growth)

# ================================
# INTERPRETATION
# ================================
# - The fixed effects model attributes variation directly to geography.
# - The PCA extracts statistical common components (factors) without labels.
# - You can interpret the PCA factors by comparing them to national/state trends.
