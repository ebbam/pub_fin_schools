...
Public schools are responsible for education over 80% of American
...
Education, the town of Greenwich town, one of the highest-income towns
...
Finance Indicators Database provides provides detailed data on School
...
cz_mean_test %>%
  ...
  labs(
    ...
    subtitle = "Scattered points: Commuting zone-level expenditure per pupil\nBlack diamond: Population-weighted state mean expenditure per pupil\nTriangle: Estimated adequate K-12 expenditure per pupil\n(School Finance Indicators Database - University of Wisconsin).\nRed (green) indicates whether the state mean is below (above) adequate spending levels."
  ) +
...
calculation in @sec-growthrates. The black dashed line represents the
...
shocks that are driven by national industry trends weighted but the
...
local outcomes and construct the former of the first-stage relationship
...
geom_vline(data = percentile_df,
  ...
  labs(x = "% Coverage of CZ's Employed by NAICS sub-categorization",
       ...
       subtitle = str_wrap("Data coverage is calculated as the fraction of total local employment accounted for in the industry-specific employment values. Percentage labels represent proportion of commuting zones (percentiles) falling below a coverage value.", width = 80),
...
instrumental variable approach. However, the Wald over-identification
test suggests potential instrument invalidity, though this is likely an
artifact of the inclusion of AR(1) terms in the first-stage and not an
indictment of the exclusion restriction itself. The implied long-run
...
sign than the GDP growth rates, indicating even potential local
divergence in growth rates. Though the calculations above could lead to
...
thresholds (F statistics \<= 12 and p-value \< 0.05) and the p-value of
...
local wages on public education expenditure is a 2.2% increase, with the
...
run_industry_analysis(
  ...
  color_palette = "Polychrome::palette36",
  plot_title = str_wrap("Effect of 1% Increase in Wage on Ed. Exp. per Pupil Using Industry-Specific Wage SS Shock", width = 80),
...
#   ind_mods <- imap_dfr(state_mods_ind, ~extract_coef_info(.x, .y, iv = TRUE)) %>%
#     rbind(iv_model) %>%
#     mutate(industry = gsub("lev_ss_", "", k)) %>%
#     rbind(., ind_mods)
#   
#   invisible(gc(verbose = FALSE))
# }
...
#   ) +
#   labs(
#     title = "Effect of 1% Increase in Wage on Ed. Exp. per Pupil Using Industry-Specific Wage SS Shock",
#     x = "Industry",
#     y = "Coefficient Estimate",
#     caption = "Effect of 1% increase in Wage Using Industry-Specific Wage (shift-share) Shocks. Controls: enrollment, GDP, intergovernmental transfers, year & CZ FE.\nWithin R² of 'All' estimation in point color.\n X reflects statistically significant state-specific treatment effects with CZ FE.",
#     fill = "Within R² (All model)",
#     color = "State"  # Add this for state colors
#   )
# library(grDevices)
...
invisible(gc(verbose = FALSE))
# 
# extract_coef_info_ind <- function(model, state_name, iv = FALSE) {
#       # Get summary
#       coef_summary <- summary(model)$coeftable
#   if(iv){
#       ftest_summary <- fitstat(model, "ivf1.stat")[[1]]
...
#   coef_df <- as.data.frame(coef_summary) %>%
#     rownames_to_column("variable") %>%
#     filter(grepl("annual_avg_wkly_wage_measure", variable)) %>%
#     mutate(
#       state = state_name,
#       # Calculate 95% confidence intervals
#       ci_lower = Estimate - 1.96 * `Std. Error`,
...
run_industry_analysis(
  ...
  color_palette = "Polychrome::palette36",
  plot_title = str_wrap("Effect of 1% Increase in Industry-Specific Wages on Education Expenditure per Pupil\nUsing GDP SS Shock", width = 80),
  plot_caption = str_wrap("Effect of 1% increase in Industry-specific Wage Using GDP (shift-share) Shocks. Controls: enrollment, GDP, intergov transfers, year & CZ FE. Within R² of full-sample estimation in point color. Each state abbreviation reflects statistically significant state-specific treatment effects with CZ FE. All points and state names displayed demonstrate treatment effects whose p-values <= 0.05 and first-stage F-statistic >= 12.", width = 120),
  geom_point_filter_all = ~ dplyr::filter(.x, state == "All" & grepl("*", significance, fixed = TRUE) & ftest >= 12)
)

# # --- 1) Run models and collect estimates --------------------------------------
# ind_mods <- tibble()
# for (k in cz_temp_vars) {
#   if(yes_contemp_ss){
#     ss_form_temp <- paste0(" log(",k, ") ~ lev_gdp_ss_2d + l1_lev_gdp_ss_2d + l2_lev_gdp_ss_2d + l(log(",k,"), 1)" )
#   }else{
#     ss_form_temp <- paste0(" log(",k, ") ~ l1_lev_gdp_ss_2d + l2_lev_gdp_ss_2d + l(log(",k,"), 1)" )
#   }
# 
#   # "All" model IV
#   iv_model_raw <- feols(as.formula(paste0(iv_lev_form, ss_form_temp)), 
#     data = df_ivs_ind, panel.id = c("unit", "year"), cluster = "unit") 
#     
#   iv_model <- iv_model_raw %>% extract_coef_info_ind(state_name = "All", iv = TRUE)
#   
#   # State-by-state models
#   state_mods_ind <- list()
#   for (s in reg_states) {
...
#   }
# 
#   ind_mods <- imap_dfr(state_mods_ind, ~extract_coef_info_ind(.x, .y, iv = TRUE)) %>%
#     rbind(iv_model) %>%
#     mutate(industry = gsub("annual_avg_wkly_wage_measure_", "", k)) %>%
#     rbind(., ind_mods)
#   
#   invisible(gc(verbose = FALSE))
# }
...
#   ) +
#   labs(
#     title = str_wrap("Effect of 1% Increase in Industry-Specific Wages on Education Expenditure per Pupil\nUsing GDP SS Shock", width = 80),
#     x = "Industry",
#     y = "Coefficient Estimate",
#     caption = str_wrap("Effect of 1% increase in Industry-specific Wage Using GDP (shift-share) Shocks. Controls: enrollment, GDP, intergov transfers, year & CZ FE. Within R² of full-sample estimation in point color. Each state abbreviation reflects statistically significant state-specific treatment effects with CZ FE. All points and state names displayed demonstrate treatment effects whose p-values <= 0.05 and first-stage F-statistic >= 12.", width = 120),
#     fill = "Within R² (All model)",
#     color = "State"  # Add this for state colors
#   )
# library(grDevices)
...
```{r indind_new_2, echo = FALSE, warning = FALSE, message = FALSE, fig.height = 8, fig.width = 8, fig.cap = "Industry-Specific Wage Effect via Industry-Specific VA SS Shock"}
invisible(gc(verbose = FALSE))
...
run_industry_analysis(
  ...
  color_palette = "Polychrome::palette36",
  plot_title = str_wrap("Effect of 1% Increase in Industry-Specific Wages on Education Expenditure per Pupil\nUsing GDP SS Shock", width = 80),
  plot_caption = str_wrap("Effect of 1% increase in Industry-specific Wage using Industry-Specific GDP (shift-share) Shocks. Controls: enrollment, GDP, intergov transfers, year & CZ FE. Within R² of full-sample estimation in point color. Each state abbreviation reflects statistically significant state-specific treatment effects with CZ FE. All points and state names displayed demonstrate treatment effects whose p-values <= 0.05 and first-stage F-statistic >= 12.", width = 120),
  geom_point_filter_all = ~ dplyr::filter(.x, state == "All" & grepl("*", significance, fixed = TRUE) & ftest >= 12)
)
...
# for (k in cz_temp_vars) {
#   ind_val <- gsub("annual_avg_wkly_wage_measure_", "", k)
#   if(ind_val %in% c("31_33", "48_49", "44_45", "92", "61")){
#     next
#   }
#   if(yes_contemp_ss){
#     ss_form_temp <- paste0(" log(annual_avg_wkly_wage_measure_", ind_val, ") ~ lev_gdp_ss_", ind_val, " + l1_lev_gdp_ss_", ind_val, " + l2_lev_gdp_ss_", ind_val, " + l(log(",k,"), 1)" )
#   }else{
#     ss_form_temp <- paste0(" log(",k, ") ~ l1_lev_gdp_ss_", ind_val, " + l2_lev_gdp_ss_", ind_val, " + l(log(",k,"), 1)")
#   }
# 
#   # "All" model IV
#   iv_model_raw <- feols(as.formula(paste0(iv_lev_form, ss_form_temp)), 
...
#     x = "Industry",
#     y = "Coefficient Estimate",
#     caption = str_wrap("Effect of 1% increase in Industry-specific Wage using Industry-Specific GDP (shift-share) Shocks. Controls: enrollment, GDP, intergov transfers, year & CZ FE. Within R² of full-sample estimation in point color. Each state abbreviation reflects statistically significant state-specific treatment effects with CZ FE. All points and state names displayed demonstrate treatment effects whose p-values <= 0.05 and first-stage F-statistic >= 12.", width = 120),
#     fill = "Within R² (All model)",
#     color = "State"  # Add this for state colors
#   )
# library(grDevices)
...
-   Though we have attempted to bypass deep endogeneity issues in the
    ...
    excludability assumptions about local industry composition are
    strong.
...
-   Revise the industry-by-industry estimation. The industry-by-industry
    ...
    *industry-specific* wages using the described industry-specific
...
-   Provide an improved calculation about the economic meaning of the
    ...
    reduce class sizes or fund salary increase for teachers, for
    example.
...
-   Consider additional outcome variables that reflect within commuting zone dynamics (i.e., inequality in expenditure).
...
with compatibility between various typesetting tools:
