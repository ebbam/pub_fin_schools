...
\textcolor{violet}{I am compiling this in Quarto Markdown and have yet to figure out how to reference my tables systematically. Please excuse lack of referencing to regression tables and figures at the moment.}
...
improvement in living standards and where inequality is not spatially
...
expenditure to local economic growth exists. If productivity gains
...
*RQ1:* Over the last twenty years, has wage inequality impacted local
public education expenditure across commuting zones? 
...
identify the effect of shocks to this instrument on local public
...
endogeneity challenge present in any study of the linkage between local
economic growth and measures of well-being. In the context of this
...
estimates mask in our main analysis. Centrally, we employ an
instrumental variable design using a VA-based shift share
instrument to identify the dependence of local public education
expenditure on local economic prosperity. We further advance this
analysis via state-by-state and industry-by-industry estimations which
allow for industry and region-specific results to emerge. Furthermore,
we group commuting zones by their historic growth trajectories to
improve comparability of treatment and control groups in our
instrumental variable design as well as emphasize context-specific
outcomes.
...
2.  We establish a causal link between public education expenditure and
    ...
    exposure treatment estimation generally, and subsequently for specific industries and states and growth cohorts.
3.  \textcolor{violet}{State-level estimation result.}
4.  \textcolor{violet}{Industry-level estimation result.}

### Value-added relative to related literature
...
We compile a panel dataset of 636 commuting zones across 40 US states
between 2001-2021 including the following metrics:
...
the remaining portion of GDP is government expenditure which includes
...
Mac since January 1975 \href{https://www.fhfa.gov/data/hpi}{(Source)}. It is
...
country using an AR(1) two-way fixed effects panel model. 
...
The Appendices provide national-level average treatment effects across all estimation strategies, for reference. 
...
rates via human capital investments. Second, in terms of public impact,
...
Slips in public education provision at such a young age could have
scarring effects. $\alpha_i$
represents either a CZ or state fixed effect and $\gamma_t$ represents
...
We provide additional robustness checks of these relationships in the Appendix including esitmation using state-level fixed effects. However, given our interest in commuting-zone specific outcomes, we proceed with commuting zone level fixed effects. 

Finally, we approach the estimation using levels. The coefficient on our AR(1) term is far from 1 which indicates that unit roots are unlikely to be present in our dependent variable.
...
capital (though, as described in the previous section, we protect against this due to our choice of primary education as the dependent variable). Therefore, we instrument local wages
...
Economically, this specification measures how much of each CZ’s growth
...
Figure 4 plots the distribution of values of $\alpha_g$ where the vertical dashed lines represent the 25th and 75th percentiles. \textcolor{red}{I recently attended a talk that noted this is a common distribution for personal income growth rates. Might be interesting to note that here.} 

Next, Figure 5 below demonstrates the considerable variability in GDP-level
...
Finally, Figure 6 represents the distribution of the loadings on the residualised state factors and the national growth rates at the commuting zone level. \textcolor{violet}{I wonder if there is a potential interpretation for the right skew of the betas. I will look into this.}
...
\textcolor{red}{We only have national level wage growth data. I have not yet implemented state-level wage growth data. So for now we only use the CZ trend, netting out the national trend. This also explains the missing sections in Figure 11 of the relevant betas.}
...
In Figure 8, we see that there is similar variability though the
...
Figure 9 where, although there is a positive correlation between
...
Figure 14 presents a correlation coefficient by commuting zone between the two
rates, providing greater detail on this relationship.
\textcolor{violet}{Georgia is an incredibly interesting case in which nearly all cmomuting zones have relatively declining GDPpc growth rates but relatively growing wage growth rates}.
...
12 demonstrates that states contain anywhere from 2 (Delaware) to 58
...
might be confounding our current treatment estimates (of course, these
...
of these estimations in Figure 13. First, we re-employ our baseline
regression in which we regress our outcome variable on wage levels at
time $t-h$ where $h = [t-0,t-2]$. The plot below displays the cumulative
...
Placing the descriptive and causal estimates side-by-side (linear combination of l0, l1, l2 lagged wage coefficient values) and the second stage regression estimate (statistically significant second-stage estimates are represented by solid blue filled circles) on instrumented local wages we can see an emerging pattern. First, positive wage-education relationships (Ohio, Arizona, New Hampshire) are corroborated more often than a negative one (WA). Among the well-identified realtionships, only Missouri's relationship is revealed to be the opposite of what is suggested by the descriptive regression results. \textcolor{red}{Note the very unusualy behaviour of Mississippi. I tried excluding Mississippi from the earlier regression estimations to ensure that it was not driving the negative relationship between wages and education expenditure. However, the strength and direction of the effect in the overall and partitioned sample regression results were consistenw even without Missouri.}

Ohio...sign matched.
Arizona...sign matched.
Washington...sign matched.
New Hampshire...sign matched.
Missouri...sign **not** matched.
...
zone sample and then grouping commuting zones by state. We instrument
local level wages using these decomposed shift-share shocks by industry.
