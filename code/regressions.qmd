...
append_to_appendix <- function(title, description_lines, float_barrier = TRUE) {
...
# Append external files (tex or md) to appendix
append_file_to_appendix <- function(file_path, title = NULL, float_barrier = TRUE) {
  if (!BUILDING_APPENDIX) return(invisible(NULL))
  ...
  # Determine file type and use appropriate inclusion method
  file_ext <- tools::file_ext(file_path)
  
  if (file_ext == "tex") {
    # LaTeX file - use \input{}
    lines <- c(
      lines,
      "```{=latex}",
      paste0("\\input{", file_path, "}"),
      "```",
      ""
    )
  } else if (file_ext == "md") {
    # Markdown file - use {{< include >}}
    lines <- c(
      lines,
      sprintf("{{< include %s >}}", file_path),
      ""
    )
  } else {
    stop("Unsupported file type. Only .tex and .md files are supported.")
  }
...
# Finalize appendix with page break
...
append_file_to_appendix("../data/raw/QCEW/data_desc_qcew_cz_ind_wage_stats.md", float_barrier = TRUE, title = "# Data")
...
may reinforce existing spatial inequalities in economic activities,
...
mobility, and long-run opportunity [@chetty2014]. Wage stagnation can
...
@obrien2025; @feler2017; @chetty2026]. Evidence of how income
inequality perpetuates other forms of inequality (opportunity, health,
infrastructure quality, and broader well-being) is steadily increasing. [@chetty2016; @logan2012; @semuels2016; @Avancena2021; @flavin2009;
...
resources spent vary widely across the country [@biolsi2022]. A
sometimes lacking, or at best under-performing, federal equalization
system perpetuates this structural force [@beland2017; @biasi2023;
@hoxby1998; @mccabe2019; @baker2016; @aesa2024]. Take Connecticut for
example. In 2016, according to the Connecticut State Department of
Education, the town of Greenwich, one of the highest-income towns
in the country, spent \$22,000 per pupil while Bridgeport, although only
located 40 kilometers away, spent \$14,000 [@semuels2016].
...
decoupling, inequality in local public goods provision, and US school
financing system, one central link remains under-explored: Are local
public education budgets sensitive to local wages, and if so, for whom
...
variation by exploiting how regions are differentially exposed
...
that *relative* growth rates are of particular importance in a landscape
where local economies determine the actual *value* of wage levels. This
allows us to identify where the wage–expenditure link is strongest or
weakest.
...
2.  Second, estimating the instrumental variable model separately for
    ...
    states analysed in this study show persistent causal relationships
    between wages and education expenditure, indicating that these carry
    the weight of national-level identification. Many of these states
    rely on an above-average share of education funding coming from
    local sources rather than intergovernmental transfers. Across these
    states the wage elasticity varies between 2-10% increases in
    response to a 10% increase in local wages, with Colorado, Florida,
    and South Dakota exhibiting highest wage elasticities.

3.  Third, elasticities are largest, both in terms of size and
...
channel, the inter-generational transmission of economic opportunity.
...
options(width = 500)
knitr::opts_chunk$set(echo = FALSE, cache = FALSE, warning = FALSE, message = FALSE, fig.width = 10, fig.height = 10, dev = "ragg_png")
...
fitstat_register("iv_fstat_marg_p", 
                ...
                "F (marg) p-value")
# Function that returns both F-stat and p-value
partial_f_wald <- function(model) {
  # Check if this is an IV model with first stage
  if (is.null(model$iv_first_stage)) {
    return(list(fstat = NA, pval = NA))
  }
  
  # Get first stage model
  fs <- model$iv_first_stage[[1]]
  
  # Define your excluded instruments
  excluded_instruments <- c("lev_gdp_ss_2d", "l1_lev_gdp_ss_2d", "l2_lev_gdp_ss_2d")
  
  # Check if instruments are in the model
  fs_vars <- names(coef(fs))
  instruments_present <- excluded_instruments[excluded_instruments %in% fs_vars]
  
  if (length(instruments_present) == 0) {
    return(list(fstat = NA, pval = NA))
  }
  
  tryCatch({
    # Suppress output from wald()
    sink(file = nullfile())
    on.exit(sink(), add = TRUE)
    
    # Get Wald test result
    wald_result <- wald(fs, "ss", cluster = "unit")
    
    # Calculate F-statistic and p-value
    #q <- length(instruments_present)
    f_stat <- wald_result$stat
    p_val <- wald_result$p #pchisq(wald_result$stat, df = q, lower.tail = FALSE)
    
    # Return both
    return(list(fstat = f_stat, pval = p_val))
...
# Register F-statistic
fitstat_register(
  "partial_f_wald",
  function(x) {
    result <- partial_f_wald(x)
    return(result$fstat)
  },
  "Partial F (Wald)"
)
...
# # Function to calculate effective F-statistic (Montiel Olea & Pflueger)
# partial_f_effective <- function(model) {
#   # Check if this is an IV model with first stage
#   if (is.null(model$iv_first_stage)) {
#     return(NA)
#   }
...
#     fs_vcov <- vcov(fs)
#     
#     # Find indices of excluded instruments
#     iv_indices <- which(names(fs_coefs) %in% excluded_instruments)
#     
#     if (length(iv_indices) == 0) {
#       return(NA)
#     }
#     
#     # Extract relevant parts
#     beta_iv <- fs_coefs[iv_indices]
#     V_iv <- fs_vcov[iv_indices, iv_indices]
#     
#     # Calculate Wald statistic
#     wald_stat <- as.numeric(t(beta_iv) %*% solve(V_iv) %*% beta_iv)
#     
#     # Effective F
#     effective_f <- wald_stat / length(iv_indices)
#     
#     return(effective_f)
#   }, error = function(e) {
#     return(NA)
...
# }
# 
# fitstat_register("partial_f_effective", 
#                  partial_f_effective, 
#                  "Effective F-stat")
...
prior to 2009 and is considered less accurate than those provided by the SEER Database.

**Adequate Education Spending**: The University of Wisconsin's School
Finance Indicators Database provides detailed data on School
Funding Adequacy, measured as the dollar estimate needed per pupil to
achieve U.S. average test scores [@baker].
...
(gdp_plot + hpi_plot) / (wage_plot + pop_plot)+ plot_annotation(title = "Key Relationships Between Economic Variables and Levels of per pupil Education Expenditure",
...
run_model(prop_taxes, mines_cz) %>%
  etable(tex = latex_tables, adjustbox = TRUE, label = "tbl_proptax_gdp", title = "Relationship between Property Tax and Local GDP")
...
what_is_cz <- mines_cz %>% select(cz_id, state, n_fips, all_states) %>% distinct
what_is_cz $state_name <- sapply(what_is_cz$state, get_state)

what_is_cz %>% 
  ggplot() + 
  geom_histogram(aes(x = n_fips), bins = 30, fill = "turquoise4", alpha = 0.7) + 
  labs(x = "Number of Constituent Counties", y = "Commuting Zone Count", title = "Number of Counties per Commuting Zone") +
  common_theme +
    theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) 

ggsave(here("commuting_zones_per_county.png"), width = 10, height = 8)

 what_is_cz %>% 
  filter(n_fips > 11) %>% 
  pull(cz_id) -> outlier_czs

temp_outlier_czs <- czs %>% filter(cz_id %in% outlier_czs) %>% select(fips, cz_id)
fips_codes %>% mutate(fips = paste0(state_code, county_code)) %>% select(fips, state_name, county) %>%  left_join(temp_outlier_czs, ., by = "fips") %>% print(n = 67)
...
# Illinois (12) - around Chicago (contains Illoinois and Wisconsin)
# MN (12) - around Minneapolis (contains Minnesotan and Wisconsin counties)
...
calculation outlined in @sec-growthrates. The black dashed line represents the
annual national wage growth rate.
...
concern between public education expenditure and wages. First, there is a
...
community exposure to this industries.[^8]
...
Finally, authors can make an additional design choice about whether
...
1-year, 2-year time lags as instruments.
...
school could likely affect local wages given such students are of or near
...
year-fixed effects, with stage-relevant superscripts. $\varepsilon_{it}$
and $u_{it}$ represents the error term of the second and first stage,
respectively.
...
c(c(run_model(paste0(baseline_sw, ' | state + year'), mines_cz)),
     c(run_model(paste0(baseline_full, ' | state + year'), mines_cz))) %>% etable(adjustbox = TRUE, tex = latex_tables, caption = "Descriptive Results in Levels", label = "tbl_desc_res_lev")

c(c(run_model(paste0(baseline_gr_sw, ' | state + year'), mines_cz)),
  c(run_model(paste0(baseline_gr_full, ' | state + year'), mines_cz))) %>% etable(adjustbox = TRUE, tex = latex_tables, caption = "Descriptive Results in Growth Rates", label = "tbl_desc_res_gr")
...
geom_vline(data = percentile_df,
  ...
  labs(x = "% Coverage of CZ's Employed by NAICS sub-categorization",
       y = "No. CZs",
       title = str_wrap("Data Coverage of Industry-level Employment as Share of Total Reported Employed", width = 60),
...
etable(iv_model_2d_lev_lev, iv_model_2d_lev_lev_test4, iv_model_2d_gr_gr, iv_model_2d_lev_lev_prop, adjustbox = TRUE, stage = 1:2,fitstat = iv_fitstats, tex = latex_tables, caption = "IV Estimation Using Wage-based Shift-share instrument (l0, l1, l2) in Levels varying state and CZ fixed effects and lags.")
...
instrumental variable approach. The implied long-run
elasticity is near 0.46, indicating that the cumulative effect of a 10%
wage increase is a 4.6% increase in education spending.
...
etable(iv_model_2d_lev_lev, iv_model_2d_lev_lev_test4, adjustbox = TRUE, stage = 1:2,fitstat = iv_fitstats, tex = latex_tables, caption = "IV Estimation Using VA-based Shift-share instrument (l0, l1, l2) in Levels with CZ and year fixed effects and lags.", label = "tbl_va_ss_baseline")

etable(iv_model_2d_lev_lev_prop, iv_model_2d_lev_lev_priv_acs5, iv_model_2d_lev_lev_priv_acs1, adjustbox = TRUE, stage = 1:2,fitstat = iv_fitstats, tex = latex_tables, caption = "IV Estimation Using VA-based Shift-share instrument (l0, l1, l2) in Levels with CZ and year fixed effects and lags.", label = "tbl_va_ss_baseline")
# 
...
manner, employ (2) state-by-state and (3) industry-by-industry estimations in our IV specifications using our VA-based shift-share
instrument.
...
<!-- \textcolor{red}{I recently attended a talk that noted this is a common distribution for personal income growth rates. Might be interesting to note that here.} -->
...
sign than the GDP per capita growth rates, indicating local
divergence between these rates. Though the calculations above could lead to
...
trend_coefs_hist <- cz_trends_comb_plot %>% 
  ...
  labs(title = "Histogram of Wage, GDP, GDPpc Growth Rates",
       x = "Growth Rate Value",
       y = "Count",
       color = "Metric",
       fill = "Metric") +
...
(trend_coefs_hist + hist_state) / (scatter_betas + hist_nat) + plot_annotation(title = "Commuting Zone Growth Rates and Jurisdictional Loadings", subtitle = "Each point or component unit of a distribution represents a single commuting zone.", theme = plot_annotation_theme)
...
```{r, echo = FALSE, fig.height = 12, fig.width = 10, warning = FALSE, fig.cap = "Correlation Between GDP Growth Rates and Wage Growth Rates by State"}
...
\autoref{tbl_gdp_ss_gdp_subsamples} partitions CZs by long-run GDP
...
Regardless, all models except Column 5 in \autoref{tbl_gdp_ss_wage_subsamples} remain well-identified, with high first stage F
...
States vary in the number of commuting zones they contain. Figure
...
states that contain less than 5 commuting zones (Delaware, Maine, Nevada, New Hampshire, New Jersey) due to sample size
concerns.
...
of the effect for 10 states: Colorado, Florida, South Dakota, Kentucky, North Dakota,
Louisiana, Pennsylvania, Oregon, Oklahoma, and
Indiana. In estimating the state-by-state regressions, we only present the results for states that exceed conventional weak instrument
thresholds (F statistic \>= 12 and p-value \< 0.05) and whose p-values for
the second stage coefficient of interest is \< 0.1.
...
extremes in these values. Notably, these states even vary in reliance on local versus intergovernmental
sources for public education expenditure.
...
### Industry by Industry {#sec-indbyind}
...
local wages on public education expenditure is approximately a 2% increase, with the
effect's magnitude varying meaningfully for several states. We plot the
...
0.05). This estimation provides a view on the strength of our chosen instrument. Regardless of instrument choice, the detected effect should be consistent. 
...
# Now call it three times with different parameters:
...
run_industry_analysis(
  ...
  color_palette = "Polychrome::palette36",
  plot_title = str_wrap("Effect of 1% Increase in Wage on Ed. Exp. per Pupil Using Industry-Specific Wage SS Shock", width = 70),
  plot_caption = str_wrap("Effect of 1% increase in Wage Using Industry-Specific Wage (shift-share) Shocks. Controls: enrollment, GDP, intergovernmental transfers, year & CZ FE. Within R² of 'All' estimation in point color. X reflects statistically significant state-specific treatment effects with CZ FE.", width = 120),
...
The previous estimation only provides additional confidence in our instrument choice, indicating that our effect is consistent across various instruments. However, we are interested in industry-specific wage elasticities. Therefore, we attempt an alternative estimation in which we instrument local *industry-specific* wages using our general shift-share instrument. More precisely, we instrument industry-specific local wages using our original shift-share instrument as designed in @eq-bartik. Figure 11 demonstrates the results of a full-sample estimation (dots) and state sub-samples (text labels of state names). We only display coefficient estimates whose p-value < 0.05 and first-stage F-statistic >= 12. This specification demonstrates that industry-level wages have different effects on education expenditure per pupil. Though the estimated effect remains positive for all full-sample estimations, states demonstrate diverse relationships that vary in both magnitude and sign. 

```{r indind_new, echo = FALSE, warning = FALSE, message = FALSE, fig.height = 8, fig.width = 8, fig.cap = "Industry-Specific Wage Effect via General VA SS Shock"}
invisible(gc(verbose = FALSE))
...
plot <- res_ivs_new %>%
    ...
    common_theme +
    theme(legend.position = "bottom", axis.text.x = element_text(angle = 90, hjust = 1)) +
    labs(title = str_wrap("Displaying Industry-by-Industry Estimation for Identified States from State-by-State Estimation", width = 80), x = "Industry", y = "Coefficient Estimate",
         caption = str_wrap("Controls: enrollment, GDP, intergovernmental transfers, year & CZ FE. Within R² of full-sample estimation in point color. Each state abbreviation reflects statistically significant state-specific treatment effects with year and CZ FE. All points and state names displayed demonstrate treatment effects whose p-values < 0.05 and first-stage F-statistic >= 12.", width = 120), fill = "Within R² (All model)", color = "State")
...
append_to_appendix("## Industry-by-Industry Estimation Using Industry-Specific GDP SS", 
                   "In the following section, we provide an additional alternative estimation to that described in @sec-indbyind of the main text. In this case, we instrument local industry-specific wages using our decomposed industry-specific estimation as designed in @eq-bartik-decomposed.")
...
run_industry_analysis(
  ...
  fill_palette = "Blues",
  color_palette = "Polychrome::palette36",
  plot_title = str_wrap("Effect of 1% Increase in Industry-Specific Wages on Education Expenditure per Pupil Using Industry-Specific GDP SS", width = 70),
...
Pennsylvania, North Dakota, Oregon, Oklahoma, and Indiana).
...
-   Data limits true consideration of non-stationarity issues.
    Estimating this model in first differences or growth rates would be
    preferable to address non-stationarity concerns as well as allow for
    asymmetric treatment estimation to distinguish between negative and
    positive wage pressures. However, this would require improvements in
    data collection of the required variables.
...
-   Finalize the industry-by-industry estimation. The main industry-by-industry estimation presented in Figure 10, by design, demonstrates the reliability of our headline coefficient estimate. In other words, our coefficient estimate is consistent for a wide variety of instruments. However, we are more interested in determining industry-specific wage elasticities. Therefore,  we provide two alternative estimations, the first of which is presented in the current draft (Figure 12) and the second in the Appendix. Rather than
    instrumenting overall local wage levels, we instrument
    *industry-specific* wages using either the general shift-share instrument designed in @eq-bartik or the industry-specific shift-share instruments as designed in @eq-bartik-decomposed. We believe that this revision could link well to the introduction in which only
    those regressions in which there is a strong first stage (i.e., GDP
    changes predict local wages well) would qualify for analysis. Though we have estimated both alternatives, the estimation in which we use the industry-specific shift-share instrument needs further data work to ensure accuracy. We therefore present the preliminary results from this estimation in the Appendix only. 
...
-   Consider additional outcome variables that reflect within commuting zone dynamics (i.e., inequality in expenditure). Possible outcome variables to consider:
    -   Within-commuting zone maximum, minimum, or variance in expenditure levels.
    -   Using the School Funding Adequacy Data presented in Figure 1, construct indicators of the within-commuting zone proportion of under-, over-, or adequately-funded school districts.
    -   Private school enrollment to measure potential crowding in and out of public education.
...
I would primarily like to thank Jennifer Castle for valuable supervision and comments throughout the course of this project. Thanks additionally to Jonas Kurle and Kasper Pedersen of the Calleva Research Centre and Stefania Innocenti for comments. 

# Use of AI

Below, we provide a summary of the tasks where AI was used. We
...
with compatibility between various typesetting tools. We did *not* use AI to generate any text for this manuscript.
