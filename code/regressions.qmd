...
if(unit_id == "cz_id"){
  ...
  mines_cz <- readRDS(here("data/out/regression_data_complete_fips.RDS")) %>% 
    mutate(unit = get(unit_id)) %>% 
      group_by(unit) %>%
    arrange(unit, year) %>% 
  mutate(across(c(starts_with("log_gdp_total") | starts_with("log_gdp_priv_ind")), ~.- dplyr::lag(., 1), .names = "diff_{.col}")) %>% 
  ungroup 
...
mines_restr_2021 <- mines_cz %>% filter(year == 2021) %>% 
  select(unit, log_Property_Tax_pp, log_Elem_Educ_Total_Exp_pp, log_gdp_priv_ind_pc)
...
for(ss in ss_oil_gas_instruments){
  ss_oil_gas_instruments_res  <- append(paste0("log_Total_Educ_Total_Exp_pp ~ log_Total_State_IG_Revenue_pp + log_Total_Fed_IG_Revenue_pp | unit + year | log_Property_Tax_pp ~ ", ss), ss_oil_gas_instruments_res)
} 
...
for(ss in ss_ff_instruments){
  ss_ff_instruments_res  <- append(paste0("log_Total_Educ_Total_Exp_pp ~ log_Total_State_IG_Revenue_pp + log_Total_Fed_IG_Revenue_pp | unit + year | log_Property_Tax_pp ~ ", ss), ss_ff_instruments_res)
} 
...
for(ss in ss_extraction_instruments){
  ss_extraction_instruments_res  <- append(paste0("log_Total_Educ_Total_Exp_pp ~ log_Total_State_IG_Revenue_pp + log_Total_Fed_IG_Revenue_pp | unit + year | log_Property_Tax_pp ~ ", ss), ss_extraction_instruments_res)
} 
...
<!-- mines_cz_solar <- left_join(mines_cz, spv, by = c("unit", "year")) %>%  -->
<!--   group_by(unit) %>%  -->
<!--   fill(pv_p_area, .direction = "down") %>%  -->
<!--   fill(pv_p_cap_ac, .direction = "down") %>%  -->
<!--   fill(pv_p_cap_dc, .direction = "down") %>%  -->
