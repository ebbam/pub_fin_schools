...
a reasonable standard of living for all as well as the reiteration of
...
the top five most unequal OECD countries [^1], but evidence of how
...
Economic history and trajectories of industrial development have
...
presence of higher-earnes in a local area contributes to higher levels
...
importance to ensure greater equality in the long-run. [^3] [^4]
...
*RQ1:* How has uneven industrial wage growth affected public education
expenditure?
...
This work employs Willamette University's Annual Government Finance
...
in the United States' @pierson . I aggregate school district measures up
...
setFixest_etable(fitstat = ~ . + ivf1 + ivf1.p + ivf2 + ivf2.p + wh + wh.p + ivwaldall + ivwaldall.p, 
                 se.below = TRUE)
...
mines_cz %>%
  ...
  mutate(across(c(Enrollment, pop_total), ~./1000)) %>%  # gdp_govt, 
          data.frame() %>%
          stargazer(digits = 0, digits.extra = 3, 
...
heterogeneity. Wherever possible, we include information about the
uncertainty surrounding our results.
...
@eq-gdp-function, @eq-wage-function, @eq-hpi-function where $h = [0,2]$
represents $h$-year time lags. We estimate all equations in levels and
growth rates.

$$
    X_{it}^{GDP} = \sum_{h=0}^{2} \beta_k^{\text{GDP}} \log(\text{GDP}_{i,t-h})
$$ {#eq-gdp-function}

$$
    X_{it}^{Wage} = \sum_{h=0}^{2} \beta_k^{\text{Wage}} \log(\text{Wage}_{i,t-h})
$$ {#eq-wage-function}

$$
    X_{it}^{HPI} = \sum_{h=0}^{2} \beta_k^{\text{HPI}} \log(\text{HPI}_{i,t-h})
$$ {#eq-hpi-function}

As demonstrated in Table X, education expenditure has a highly relevant
time dependence. The effect of increases in GDP two years prior has the
greatest effect on current education expenditure, implying a delayed
effect of commuting zone-level economic growth on public education
expenditure. Across all specifications, the results underscore two
important facts: the central role of intergovernmental transfers and
broader local economic fundamentals in shaping patterns of education
spending.
...
increase in per-pupil education spending, controlling for unit and year
fixed effects. This finding highlights the importance of state and
...
spending. The magnitude of the coefficients increases with the number of
...
is associated with a 0.14% increase in per-pupil spending.

The house price index also enters positively and significantly in
contemporaneous and short-lag specifications (up to t–3) underscoring
...
expected), largely confirm the patterns observed in the level
...
materialize in local education budgets. Growth in house prices is also
associated with increases in education spending which hints at the
...
state-level tax regimes and policies that govern education.
...
) + log_real_Total_IG_Revenue_pp + log_Enrollment | unit + year), mines_cz) %>% etable(adjustbox = TRUE, tex = latex_tables)
...
run_model(c(diff_log_real_Elem_Educ_Total_Exp_pp ~ sw(
  ...
  gr_hpi + l1_gr_hpi + l2_gr_hpi + l3_gr_hpi + l4_gr_hpi+ l1_diff_log_real_Elem_Educ_Total_Exp_pp) + diff_log_real_Total_IG_Revenue_pp + diff_log_Enrollment | unit + year), mines_cz) %>% etable(adjustbox = TRUE, tex = latex_tables)
...
run_model(selected_twfe_models_state_share_interaction, mines_cz) %>% etable(adjustbox = TRUE, tex = latex_tables, order = c("GDP", "Wage", "House Price"))
...
reliance of local education expenditure on economic conditions, this
...
\textcolor{red}{Will add appropriate reference.}) provides an avenue for
...
diagram of this econometric specification in \ref{fig:iv-approach.
...
pupil, driving up property values.
...
where $S_{ij\tau}$ is the local share of unit $i$'s economy (potentially
...
shares are only exogenous at a given base year/period and that national
...
across 2001-2005 as our 'base year'. [^9]
...
The below table displays the 2-digit NAICS industrial categories we use
to construct our shift-share instrument. The plot immediately following
displays the remaining issues with data coverage indicating what
proportion of total commuting zone level employment is represented by
the industry-specific data points (industry-level disaggregation
requires the suppression of certain data). Given the high degree of
missingness in the 3-digit categorisation we proceed with the 2-digit
NAICS codes in the rest of the work.
...
increase in the shift-share measure (which can be interpreted as a logarithm) is associated with a 0.05-0.5% increase
...
instrument relevance. However, the second-stage results are only significant in the case in which we include state-fixed effects rather than commuting zone level fixed effects. 

In specifications that include year and state fixed effects (Columns 3, 4, 7, and 8), we find a statistically significant and positive effect of wage shocks on education spending. However, once we include commuting zone fixed effects in place of state fixed effects (Columns 1, 2, 5, 6), the second-stage coefficients become statistically insignificant, even though the first-stage remains strong.

This suggests that the estimated effect is driven primarily by cross-sectional variation across commuting zones within states, rather than by within-CZ variation over time. In other words, places with persistently higher wage shocks also tend to have higher education spending, but temporal changes in wage shocks within a given CZ are not reliably associated with changes in spending. These results emphasize that the cross-sectional component of the variation (e.g., differences between CZs in long-run exposure to wage shocks) may be driving the overall effect. If policy relevance hinges on place-based differences, then the specification with state fixed effects may better reflect the real-world variation of interest.
...
etable(iv_model_2d_lev_lev, iv_model_2d_lev_lev_state_fe, iv_model_2d_lev_lev_l1, iv_model_2d_lev_lev_l1_state_fe, adjustbox = TRUE, stage = 1:2, tex = latex_tables)
etable(iv_model_2d_lev_gr, iv_model_2d_lev_gr_state_fe, iv_model_2d_lev_gr_l1, iv_model_2d_lev_gr_l1_state_fe, adjustbox = TRUE, stage = 1:2, tex = latex_tables)

```

#### Industry-level GDP

The first regression tables investigate the relationship between
real GDP per capita in the private industry and house prices. In the
log-level specification, real GDP (including current and lagged values
up to 7 periods) is positively and significantly associated with house
prices. The contemporaneous coefficient is large and highly significant,
and several lags (especially lags 1, 2, 3, and 5) are also positive and
statistically significant, indicating a cumulative and persistent effect
of GDP on housing values.
...
significant positive association (0.0073) though this could likely be a
...
etable(iv_model_2d_lev_lev, iv_model_2d_lev_lev_state_fe, iv_model_2d_lev_lev_l1, iv_model_2d_lev_lev_l1_state_fe, adjustbox = TRUE, stage = 1:2, tex = latex_tables, title ="GDP-based Shift-share instrument (Level-Level, varying state and CZ fixed effects and lags.")
etable(iv_model_2d_lev_gr, iv_model_2d_lev_gr_state_fe, iv_model_2d_lev_gr_l1, iv_model_2d_lev_gr_l1_state_fe, adjustbox = TRUE, stage = 1:2, tex = latex_tables,  title ="GDP-based Shift-share instrument (Level-GR, varying state and CZ fixed effects and lags.")
...
shift-share instrument, likely comes from masked 
heterogeneity across states and commuting zones.
...
growth and decline to (1) partition our sample according to metrics of economic health, employ (2) state-by-state
and (2) industry-by-industry estimations using our baseline descriptive
...
growth rates conditional on state and national level growth rates and partition our sample after this distribution.
...
trend_hist <- cz_trends %>%
  ...
  annotate("text", x = percentiles$p25trend, y = 70, label = "25th Percentile", color = my_color[2], angle = 90, vjust = -0.5) +
  annotate("text", x = percentiles$p75trend, y = 70, label = "75th Percentile", color = my_color[2], angle = 90, vjust = 1.5) +
  theme_minimal()
...
trend_pc_hist <- cz_trends %>%
  ...
  labs(x = "CZ Trend Coefficient", title = "Distribution of CZ GDP pc Trend Coefficients", y = "Frequency") +
  annotate("text", x = percentiles$p25trend_pc, y = 65, label = "25th Percentile", color =  my_color[4], angle = 90, vjust = -0.5) +
  annotate("text", x = percentiles$p75trend_pc, y = 65, label = "75th Percentile", color =  my_color[4], angle = 90, vjust = 1.5) +
  theme_minimal()
...
tmp <- colorRampPalette(brewer.pal(9, "Oranges"))(n_states + 10)  # oversample
myOranges <- tmp[-c(1:10)]   # drop the 2 lightest shades
...
for(k in list(form)){ # gr_form)){ #prop_vals_form, gr_prop_vals_form)){
  ...
  etable(mods_list, tex = latex_tables, adjustbox = TRUE, headers = rep(names(mods_list), each = n_forms), title = "Baseline Regression Applied to Declining GDP vs. Growing GDP Regions") %>% print(.)
}
...
Wages.
\textcolor{red}{I have not figured out how to suppress the R2 and within R2 of the second-stage regressions. One should only pay attention to the IV diagnostic tests in the second-stage regression.}

Result: The baseline estimate of an l-1 effect on public education expenditure is entirely dominated by declining regions. Interestingly, sample division by GDP growth rates isolates the wage-based SS instrumetn effect almost entirely indicating that wage changes in declining regions matter most. Potentially an indicator of property price spirals.
...
etable(mods, tex = latex_tables, adjustbox = TRUE, headers = names(mods), title = "Wage-based Shift-Share Instrument (l1) Applied to Declining GDP vs. Growing GDP Regions") 
etable(mods_wage, tex = latex_tables, adjustbox = TRUE, headers = names(mods_wage), title = "Wage-based Shift-Share Instrument (l1) Applied to Declining Wage vs. Growing Wage Regions")
...
Result: In the case of the GDP-based instrument, the sub-sampling procedure indicates that GDP growth has an effect on public education expenditure in all but hyper-growing areas indicating the GDP growth translates to changes in public expenditure more directly in all but non-superstar regions. 

```{r, echo = FALSE, cache = TRUE, results = 'asis'}

etable(mods_wage, tex = latex_tables, headers = names(mods_wage), adjustbox = TRUE, title = "GDP-based Shift-Share Instrument (l1) Applied to Declining Wage vs. Growing Wage Regions", label = "tbl:wss_subsamples")
etable(mods, tex = latex_tables, adjustbox = TRUE, headers = names(mods), title = "GDP-based Shift-Share Instrument (l1) Applied to Declining GDP vs. Growing GDP Regions") 
...
etable(list(baseline, wage_ss, gdp_ss), headers = c("Baseline 1", "Baseline 2", "Baseline 3", "Wage-based SS", "GDP-based SS"), adjustbox = TRUE, tex = latex_tables, title = "Outlier Trimming Results")
...
for(k in reg_states){
  mod <- dat_temp %>%
    filter(state == k) %>%
    feols(as.formula("log_real_Elem_Educ_Total_Exp_pp ~ log_real_Total_IG_Revenue_pp + log_real_gdp_priv_ind_pc + log_Enrollment + l1_log_real_Elem_Educ_Total_Exp_pp | unit + state | log_hpi ~ l(ss_2d,1)"),
    data = ., panel.id = c("unit", "year"))
  state_mods_ss[[as.character(k)]] <- mod
}
...
temp_vars <- ss_temp %>% select(starts_with("gdp_ss") & !contains("2d")) %>% names
...
for(k in temp_vars){
    ...
    # IV regression using shift-share instrument
    iv_model <- feols(
      as.formula(paste0("log_real_Elem_Educ_Total_Exp_pp ~ log_real_Total_IG_Revenue_pp + log_real_gdp_priv_ind_pc + log_Enrollment + l1_log_real_Elem_Educ_Total_Exp_pp | state + year | log_hpi ~ l(", k, ")")),
     ...
     if(tmp %>%
      select(state, unit, year, !!k) %>%
        ...
        filter(!all(is.na(get(k)))) %>% n_groups(.) == 1){
           ...
           mod <- tmp %>%
        feols(as.formula(paste0("log_real_Elem_Educ_Total_Exp_pp ~ log_real_Total_IG_Revenue_pp + log_real_gdp_priv_ind_pc + log_Enrollment  + l1_log_real_Elem_Educ_Total_Exp_pp | unit + year | log_hpi ~ l(", k, ")")),
        data = ., panel.id = c("unit", "year"))
...
# WHY ARE WE MISSING MANUFACTURAING ETC HERE!
temp_vars <- ss_temp %>% select(starts_with("ss") & !contains("2d") & !contains("3d")) %>% names
...
# geom_point(aes(y = estimate), shape = 21, size = 4, fill = NA, color = "black", stroke = 0.25, alpha = 1) +
  ...
  labs(
    ...
    x = "Industry",
    y = "Coefficient Estimate",
    caption = "Effect of 1% increase in Industry-Specific Wage (measured as shift-share instrument) on Education\n Expenditure controlling for enrollment, GDP, and intergovernmental transfers, AR(1) term, year and state-level fixed effects.\n Within R^2 of estimation reflected in color of point).",
    color = "Within R2 of State-level Estimation",
    alpha = "Statistical Significance"
  ) +
    #scale_x_discrete(labels = axis_labels)  +
