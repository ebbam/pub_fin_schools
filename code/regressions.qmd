...
abstract: |
  Over recent decades, the United States has experienced increasingly divergent patterns of economic and wage growth. Wage growth is a key driver of local wealth accumulation, enabling greater household and community investment in public goods. Regions whose wages track productivity gains likely benefit from broader economic growth, accumulating wealth that sustains local public services, while lagging regions risk weaker savings rates and eroding fiscal capacities. We estimate how divergent wage growth across U.S. regions affects public education expenditure using a shift-share instrumental variable design. Accounting for state-level heterogeneity in tax regimes, income, and economic growth, we find substantial variation in the elasticity of education spending to wages: the magnitude and statistical significance differ markedly across states. This heterogeneity explains the majority of spending responses and reflects both uneven economic fundamentals and diverse fiscal institutions that generate inequality in public goods provision. Our results demonstrate that average treatment effects in panel studies can obscure critical policy-relevant variation. The findings suggest that federal and local policies assuming uniform wage-spending relationships may fail to address or even exacerbate pervasive spatial inequality in educational investment. 
format:
  pdf:
...
and wage growth has emerged in the United States ( @hoffmann2020,
@stansbury, @economicpolicyinstitute).** While labour productivity has
continued to rise, the earnings of typical workers have increased far
more slowly, leading to a substantial decoupling between the two trends.
Economists cite various contributors to this phenomenon from human
capital (@autor2014, @katz1992) to competition (@stansbury, @deb2022,
@yeh2022, @azar2022, @autor2013a , @wilmers2018) to institutional and
policy forces (@egger2019, @lee1999, @mishel2021, @barkai2020,
@autor2016, @card2001).[^1] Though its causes are hotly debated, the
fact itself is well-documented, especially for lower- and middle-income
workers. [^2]

[^1]: Summers and Stansbury (2018) argue that productivity growth still
    ...
    the workforce. They point to declining union density, erosion of the
    minimum wage, globalization, and increased market concentration as
    key factors that have shifted bargaining power away from workers and
    reduced labour’s share of national income @autor2014. Furthermore,
    additional evidence finds that this decoupling is far from a
    universal phenomenon. Rather, decoupling applies almost strictly to
    lower- and medium-wage earners, while already higher wages manage to
    keep up (relatively) with productivity growth rates.
...
**The direct consequences of this decoupling are clear.** Though
...
@Avancena2021, @flavin2009). @boustanEffectRisingIncome2013 find that
greater income inequality leads to higher public expenditure across all
public goods indicating that a presence of higher-earners in a local
area contributes to higher levels of expenditure. Though this does not
support an unambiguous denunciation of inequality in itself, it provides
additional evidence for the fact that local incomes affect public
expenditure raising the potential for "superstar" and "left behind"
regions to emerge absent even income growth.
...
of paramount importance to ensure greater equality in the long-run. [^3]
[^4] \[\^6\] Altogether, this evidence points to the value of
...
**Expenditure numbers matter for education quality/outcomes.** (Baker
2016, Jackson 2016),

**This study therefore determines whether elasticities of public
...
*RQ1:* Do local wage gains affect public education expenditure?

*RQ2:* If so, is this relationship constant across commuting zones? What
sources of heterogeneity mediate this relationship?

*RQ3:* In light of reforms to intergovernmental education funding to
alleviate uneven educational expenditure, do intergovernmental transfers
alleviate wealth-driven inequalities in public education expenditure?
...
real value added. Using data from the U.S. Bureau of Labor Statistics
and Bureau of Economic Analysis, we construct commuting-zone-level
Bartik instruments based on both outcomes. This provides a credible and
transparent identification strategy that links macroeconomic shocks to
...
variable design.
<!-- \textcolor{violet}{We then provide an analysis of the relationship between diverse treatment effects and commuting zone characteristics. } -->

## Detailed results

1.  Public education expenditure is not agnostic to local economic
    conditions. We establish a strong causal link between public
    education expenditure and local wages using a shift-share
    instrumental variable estimation, and subsequently for specific
    industries and states and growth cohorts. The estimated treatment
    effect of local wages on public education expenditure varies in
    magnitude, and statistical significance across states.
2.  Estimating the instrumental variable model separately for each state
...
3.  \textcolor{violet}{Industry-level estimation result.} TBA
...
data for property taxes collected used in the regressions below also
come from this dataset. Expenditure on vocational training and from
...
**Property Prices:** The US Federal Housing Finance Agency provides
...
[^6]: The reason 13% of all US CZs are missing from the dataset is
    because of (1) the exclusion criteria already outlined; (2) Hawaii
    and Alaska have been excluded due to the methodological challenge of
...
# test_panel_stationarity <- function(pdata, var_name, test_type = "ips") {
#   tryCatch({
#     result <- purtest(pdata[[var_name]], 
#                      test = test_type,
#                      exo = "intercept",  # use "trend" if variables have trends
#                      lags = "AIC",
#                      pmax = 4)
#     
#     return(list(
...
#     ))
#   }, error = function(e) {
#     return(list(statistic = NA, p_value = NA, is_stationary = NA))
#   })
# }
# 
...
#                   "real_gdp_priv_ind_pc", "hpi", "pct_black", 
#                   "pct_white", "pct_hispanic")
# 
# # Test levels
# level_results <- lapply(vars_to_test, function(v) {
#   test_panel_stationarity(mines_cz, v)
# })
# names(level_results) <- vars_to_test
# 
# # For variables non-stationary in levels, test first differences
# integration_orders <- sapply(vars_to_test, function(v) {
#   if (level_results[[v]]$is_stationary) {
#     return("I(0)")
#   } else {
#     # Test first difference
#     pdata[[paste0("d_", v)]] <- diff(pdata[[v]])
#     diff_test <- test_panel_stationarity(pdata, paste0("d_", v))
#     
#     if (diff_test$is_stationary) {
#       return("I(1)")
#     } else {
#       return("I(2)")  # or test second difference if needed
#     }
#   }
# })
# 
...
#   Integration_Order = integration_orders,
#   Level_p_value = sapply(level_results, function(x) round(x$p_value, 3))
# )
# 
# print(results_df)
...
![Wages and Productivity](../output/wage_trends_plot.png)
...
real value added represented in @eq-bartik. $G_{njt}$ represents
...
iv_lev_form <- "log_real_Elem_Educ_Total_Exp_pp ~  log_real_Total_IG_Revenue_pp + log_real_gdp_priv_ind_pc + log_Enrollment + pct_black + pct_hispanic | unit + year | "
...
ss_lev_gr_va <- "log_weighted_annual_avg_wkly_wage ~ gdp_ss_2d + l1_gdp_ss_2d + l2_gdp_ss_2d"

ss_gr_gr_va <- "gr_weighted_annual_avg_wkly_wage ~ gdp_ss_2d + l1_gdp_ss_2d + l2_gdp_ss_2d"

ss_lev_lev_va <- "log_weighted_annual_avg_wkly_wage ~ lev_gdp_ss_2d + l1_lev_gdp_ss_2d + l2_lev_gdp_ss_2d"
...
iv_model_2d_lev_gr <- run_manual_iv(iv_formula = paste0(iv_lev_form, " log_weighted_annual_avg_wkly_wage ~ ss_2d + l1_ss_2d + l2_ss_2d"), ss_formula = "log_weighted_annual_avg_wkly_wage ~ ss_2d + l1_ss_2d + l2_ss_2d",
    data = df_ivs, panel.id = c("unit", "year"), cluster = "unit", bootstrap_se = bootstrap_se_spec, n_boot = n_boots, parallel = parallel_spec)

iv_model_2d_gr_gr <- run_manual_iv( iv_formula = paste0("diff_log_real_Elem_Educ_Total_Exp_pp ~  l1_diff_log_real_Elem_Educ_Total_Exp_pp + diff_log_real_Total_IG_Revenue_pp + diff_log_real_gdp_priv_ind_pc + diff_log_Enrollment | unit + year | ", "gr_weighted_annual_avg_wkly_wage ~ ss_2d + l1_ss_2d + l2_ss_2d"), ss_formula = "gr_weighted_annual_avg_wkly_wage ~ ss_2d + l1_ss_2d + l2_ss_2d", data = df_ivs, panel.id = c("unit", "year"), cluster = "unit", bootstrap_se = bootstrap_se_spec, n_boot = n_boots, parallel = parallel_spec)


iv_model_2d_lev_lev <- run_manual_iv(paste0(iv_lev_form, " log_weighted_annual_avg_wkly_wage ~ lev_ss_2d + l1_lev_ss_2d + l2_lev_ss_2d"), ss_formula = " log_weighted_annual_avg_wkly_wage ~ lev_ss_2d + l1_lev_ss_2d + l2_lev_ss_2d",
    data = df_ivs, panel.id = c("unit", "year"), cluster = "unit", bootstrap_se = bootstrap_se_spec, n_boot = n_boots, parallel = parallel_spec)
...
# iv_model_2d_lev_gr_state_fe <- run_manual_iv(   iv_formula = paste0(iv_lev_form, " | state + year | log_weighted_annual_avg_wkly_wage ~ l(gdp_ss_2d,0:2)")),
#     data = df_ivs, panel.id = c("unit", "year"), cluster = "unit")
# 
...
#     data = df_ivs, panel.id = c("unit", "year"), cluster = "unit")
# 
# iv_model_2d_lev_gr_l1_state_fe <- run_manual_iv(   iv_formula = paste0(iv_lev_form, " | state + year | log_weighted_annual_avg_wkly_wage ~ l(gdp_ss_2d,0:2)")),
#     data = df_ivs, panel.id = c("unit", "year"), cluster = "unit")
# 
...
iv_model_2d_lev_gr <- run_manual_iv(iv_formula = paste0(iv_lev_form, ss_lev_lev_va), ss_formula = ss_lev_lev_va,
    data = df_ivs, panel.id = c("unit", "year"), cluster = "unit", bootstrap_se = bootstrap_se_spec, n_boot = n_boots, parallel = parallel_spec)

iv_model_2d_gr_gr <- run_manual_iv(iv_formula = paste0(iv_gr_form, ss_gr_gr_va), ss_formula = ss_gr_gr_va, data = df_ivs, panel.id = c("unit", "year"), cluster = "unit", bootstrap_se = bootstrap_se_spec, n_boot = n_boots, parallel = parallel_spec)

iv_model_2d_gr_lev <- run_manual_iv(iv_formula = paste0(iv_gr_form, ss_gr_lev_va), ss_formula = ss_gr_lev_va, data = df_ivs, panel.id = c("unit", "year"), cluster = "unit", bootstrap_se = bootstrap_se_spec, n_boot = n_boots, parallel = parallel_spec)

iv_model_2d_gr_lev_lev_ed <- run_manual_iv(iv_formula = paste0(iv_lev_form, ss_gr_lev_va), ss_formula = ss_gr_lev_va, data = df_ivs, panel.id = c("unit", "year"), cluster = "unit", bootstrap_se = bootstrap_se_spec, n_boot = n_boots, parallel = parallel_spec)

iv_model_2d_lev_lev <- run_manual_iv(iv_formula = paste0(iv_lev_form, ss_lev_lev_va), ss_formula = ss_lev_lev_va, data = df_ivs, panel.id = c("unit", "year"), cluster = "unit", bootstrap_se = bootstrap_se_spec, n_boot = n_boots, parallel = parallel_spec)
# No AR
...
etable(iv_model_2d_lev_lev, iv_model_2d_gr_gr, adjustbox = TRUE, stage = 1:2,fitstat = iv_fitstats, tex = latex_tables, caption = "IV Estimation Using VA-based Shift-share instrument (l0, l1, l2) in Levels with CZ and year fixed effects and lags.", label = "tbl_va_ss_baseline")
...
<!-- \textcolor{violet}{The switch in significance between enrollment and private industry GDP contributions is really interesting. Wonder if there is an interesting interpretation here?}. -->
Most importantly though, the negative relationship between annual wages
and elementary education expenditure is only present in areas that
exhibit historically hyper-declining or moderately growing wage trends.
<!-- \textcolor{violet}{Again, this might change once I incorporate the state-level trends into the calculation of $\alpha_w$.}. -->
Additionally, the first-stage F statistics only support causal
identification in these columns.
...
for(df in names(dfs)){
    ...
    iv_model <-  run_manual_iv(iv_formula = paste0(iv_lev_form, ss_lev_lev_va), ss_formula = ss_lev_lev_va,
   data = dfs[[df]], panel.id = c("unit", "year"), cluster = "unit", include_lag_endog = TRUE, include_lag_outcome = TRUE, bootstrap_se = bootstrap_se_spec, n_boot = n_boots, parallel = parallel_spec)
...
for(df_wage in names(dfs_wage)){
    # IV regression using shift-share instrument
    iv_model <- run_manual_iv(iv_formula = paste0(iv_lev_form, ss_lev_lev_va), ss_formula = ss_lev_lev_va,
   data = dfs_wage[[df_wage]], panel.id = c("unit", "year"), cluster = "unit", bootstrap_se = bootstrap_se_spec, n_boot = n_boots, parallel = parallel_spec)
...
etable(mods_fs, tex = latex_tables, adjustbox = TRUE, headers = names(mods), fitstat = iv_fitstats, title = "First-Stage: Wage-based Shift-Share Instrument (l1) Applied to Declining GDP vs. Growing GDP Regions")
...
etable(mods_wage_fs, tex = latex_tables, adjustbox = TRUE, headers = names(mods_wage), fitstat = iv_fitstats, title = "First-Stage: Wage-based Shift-Share Instrument (l1) Applied to Declining Wage vs. Growing Wage Regions")
...
for(df in names(dfs)){
    # IV regression using shift-share instrument
    iv_model <- run_manual_iv(iv_formula = paste0(iv_lev_form, ss_lev_lev_va), ss_formula = ss_lev_lev_va,
   data = dfs[[df]], panel.id = c("unit", "year"), cluster = "unit", bootstrap_se = bootstrap_se_spec, n_boot = n_boots, parallel = parallel_spec)
    mods[[df]] <- iv_model$second_stage
    mods_fs[[df]] <- iv_model$first_stage
    gc()
}
...
etable(mods_wage_fs, tex = latex_tables, headers = names(mods_wage), adjustbox = TRUE, fitstat = iv_fitstats, title = "First-Stage: VA-based Shift-Share Instrument (l1) Applied to Declining Wage vs. Growing Wage Regions", label = "tbl_gdp_ss_wage_subsamples")
...
etable(mods_fs,   tex = latex_tables, adjustbox = TRUE, headers = names(mods), fitstat = iv_fitstats, title = "First-Stage: VA-based Shift-Share Instrument (l1) Applied to Declining GDP vs. Growing GDP Regions", label = "tbl_gdp_ss_gdp_subsamples")
...
for(k in reg_states){
  mod <- df_ivs %>%
    filter(state == k) %>%
    run_manual_iv(paste0(iv_lev_form, ss_lev_lev_va), ss_formula = ss_lev_lev_va,
    data = ., panel.id = c("unit", "year"), cluster = "unit", bootstrap_se = bootstrap_se_spec, n_boot = n_boots, parallel = parallel_spec)
  state_mods_ss[[as.character(k)]] <- mod$second_stage
  gc()
}

# Define variable names of interest
vars_of_interest <- c("log_weighted_annual_avg_wkly_wage", "fitted_endog")
...
for (k in temp_vars) {
  ...
  # "All" model IV
  iv_model <- run_manual_iv(paste0(iv_lev_form, ss_form_temp), ss_formula = ss_form_temp,
    data = df_ivs_ind, panel.id = c("unit", "year"), cluster = "unit", 
    bootstrap_se = bootstrap_se_spec, n_boot = n_boots, parallel = parallel_spec) %>% .$second_stage %>% extract_coef_info(state_name = "All", iv = TRUE)
  ...
  for (s in reg_states) {
    ...
    if (tmp %>%
      ...
      mod <- tmp %>% run_manual_iv(paste0(iv_lev_form, ss_form_temp), ss_formula = ss_form_temp,
          data = ., panel.id = c("unit", "year"), cluster = "unit", bootstrap_se = FALSE
        )
      state_mods_ind[[s]] <- mod$second_stage
    }
  }
