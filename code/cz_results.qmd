...
mines_df <- readRDS(here('data/temp/mines_restr.RDS')) %>% 
  ...
  select(year, fips, state, state_name, time, panel, 
         ...
         log_real_Total_Revenue,
         log_real_Total_IG_Revenue,
         log_real_Total_Rev_Own_Sources,
         log_real_Total_Fed_IG_Revenue, 
         log_real_Total_State_IG_Revenue,
         log_real_Tot_Local_IG_Rev, 
         Total_Taxes, 
         real_Total_Taxes, 
         Property_Tax,
         real_Property_Tax,
...
mines_cz %>% 
  ggplot(aes(x = Enrollment/n_schools, y = log_real_Elem_Educ_Total_Exp_pp, color = year)) +
  geom_jitter() +
  labs(x = "avg. Enrollment per school", y = "Elem Expenditure (log kUSD)", title = "Expenditure by Average CZ Enrollment per School") +
  theme(legend.position = "none", panel.grid.major = element_blank(), panel.grid.minor = element_blank())
